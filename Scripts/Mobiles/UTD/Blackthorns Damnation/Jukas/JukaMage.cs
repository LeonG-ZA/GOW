            ...
            if (DateTime.UtcNow >= this.m_NextAbilityTime)
            {
                ...
                if (toBuff != null)
                {
                    if (this.CanBeBeneficial(toBuff) && toBuff.BeginAction(typeof(JukaMage)))
                    {
                        ...
                        if (toScale > 0)
                        {
                            toBuff.HitsMaxSeed += ItemAttributes.Scale(toScale, 75);
                            toBuff.Hits += ItemAttributes.Scale(toScale, 75);
                        }

                        toScale = toBuff.RawStr;

                        if (toScale > 0)
                            toBuff.RawStr += ItemAttributes.Scale(toScale, 50);

                        toScale = toBuff.RawDex;

                        if (toScale > 0)
                        {
                            toBuff.RawDex += ItemAttributes.Scale(toScale, 50);
                            toBuff.Stam += ItemAttributes.Scale(toScale, 50);
                        }
