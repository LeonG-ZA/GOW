    ...
    public class RotWorm : BaseCreature
    {
		...
		public static bool IsDiseased( Mobile m )  
 		...
 		{  
 			...
 			if ( !m_BloodDiseaseTable.ContainsKey( attacker ) && InRange( attacker, 1 ) && 0.25 > Utility.RandomDouble()/* && !FontOfFortune.HasBlessing( attacker, FontOfFortune.BlessingType.Protection )*/ )  
 			{  
		...
		{  
 		...
 		{  
 			...
 			{  
 				...
 				{  
 					ItemAttributes.Damage( m_Victim, Utility.RandomMinMax( 10, 20 ), 0, 0, 0, 100, 0 );  
