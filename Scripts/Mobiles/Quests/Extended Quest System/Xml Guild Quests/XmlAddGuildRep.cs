...
{
    public class XmlAddGuildRep : XmlAttachment
    {
        ...
        public XmlAddGuildRep(ASerial serial)            : base(serial)
        {
        }

        [Attachable]
        public XmlAddGuildRep(int value, string guildType)
        {
            Value = value;
            GuildType = guildType;
        }
            ...
            if (AttachedTo is PlayerMobile)
            {
                ...
                if (list.Count > 0)
                {
                    for (int i = 0; i < list.Count; i++)
                    {
                        ...
                        {
                            list[i].Rep += Value;
                            ((Mobile)AttachedTo).SendMessage("You have advanced your standing with the {0} Guild by {1}.", list[i].GuildType, Value);
                            break;
                        }
                    }
                }
