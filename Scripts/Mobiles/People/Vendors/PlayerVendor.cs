...
namespace Server.Mobiles
{
    ...
    public class PlayerVendor : Mobile
    {
        ...
        {
            var items = this.GetEquippedItems().Where(item => item.Movable && item != this.Backpack);

            return items.Union(this.Backpack.Items.ToArray());
        }
         */
        ...
        public IEnumerable<VendorItem> GetAllVendorItems()
        {
            return m_SellItems.Values.Cast<VendorItem>();
        }
        ...
        private VendorItem SetVendorItem(Item item, int price, string description, DateTime created)
        {
            ...
            VendorItem vi = new VendorItem(this, item, price, description, created);
