	...
	public class MonsterHelper
	{
		public static Mobile GetTopAttacker( BaseCreature bc )
		{
			...
			for ( int i = 0; i < rights.Count; i++ )
			{
                ...
                if (ds.m_HasRight && ds.m_Mobile.Alive)
                    return ds.m_Mobile;
			}
					...
					else
					{
						...
						if ( map != null && caster != null )
						{
							...
							while ( m_Queue.Count > 0 )
							{
								...
								ItemAttributes.Damage( m, caster, Utility.RandomMinMax( 15, 20 ), 0, 100, 0, 0, 0 );
								m.PlaySound( 0x5CF );
							}
						}
					}
		...
		public static void StygianFireball( Mobile from, Mobile target, int hue, int itemId, int phys, int fire, int cold, int pois, int nrgy )
		{
			...
			Timer.DelayCall( TimeSpan.FromSeconds( 0.5 ), new TimerCallback(
				delegate
				{
					...
					ItemAttributes.Damage( target, Utility.RandomMinMax( 200, 250 ), phys, fire, cold, pois, nrgy );
				}
				) );
		}
		#endregion
