            ...
            for (int i = 0; i < targets.Count; i++)
            {
                ...
                switch (type)
                {
                    case ResistanceType.Physical:
                        ItemAttributes.Damage(m, (from == null) ? m : from, auradamage, 100, 0, 0, 0, 0);
                        break;
                    case ResistanceType.Fire:
                        ItemAttributes.Damage(m, (from == null) ? m : from, auradamage, 0, 100, 0, 0, 0);
                        break;
                    case ResistanceType.Cold:
                        ItemAttributes.Damage(m, (from == null) ? m : from, auradamage, 0, 0, 100, 0, 0);
                        break;
                    case ResistanceType.Poison:
                        ItemAttributes.Damage(m, (from == null) ? m : from, auradamage, 0, 0, 0, 100, 0);
                        break;
                    case ResistanceType.Energy:
                        ItemAttributes.Damage(m, (from == null) ? m : from, auradamage, 0, 0, 0, 0, 100);
                        break;
                }
            }
        ...
        public static int UseBandage(BaseCreature from, bool healmaster)
        {
            ...
            if (from.Controlled && from.ControlMaster != null && from.Hits >= (from.Hits / 2) && healmaster)
            {
                if (from.InRange(from.ControlMaster, 2) && from.ControlMaster.Alive && from.ControlMaster.Hits < from.ControlMaster.HitsMax)
                    BandageContext.BeginHeal(from, from.ControlMaster, null);
            }
            else if (from.Hits < from.HitsMax)
            {
                BandageContext.BeginHeal(from, from, null);
            }
        ...
        public static void EchoStrike(Mobile from, int min, int max)
        {
            ...
            for (int i = 0; i < mobiles.Count; i++)
            {
                ...
                if (Utility.Random(5) == 0)
                {
                    ...
                    ItemAttributes.Damage(m, from, Utility.RandomMinMax(min, max), 0, 0, 0, 0, 100);
                ...
                else
                {
                    ...
                    {
                        ItemAttributes.Damage(from, from, Utility.RandomMinMax(min, max), 0, 0, 0, 0, 100);
