...
namespace Server.Mobiles
{
	...
	public abstract class BaseVendor : BaseCreature, IVendor
	{
        ...
        private const int MaxSell = NpcConfig.NpcMaxSell;
        ...
        private static int BribeLimitCount = NpcConfig.NpcBribeLimitCount;
        private static double BribePriceRate = NpcConfig.NpcBribePriceRate;

        public int BODamount { get { return m_BODamount; } set { m_BODamount = value; } }
        public bool BODexceptional { get { return m_BODexceptional; } set { m_BODexceptional = value; } }
        public bool BODisSmith { get { return m_BODisSmith; } set { m_BODisSmith = value; } }
        public bool BODisLarge { get { return m_BODisLarge; } set { m_BODisLarge = value; } }
        public bool BODisBase { get { return m_BODisBase; } set { m_BODisBase = value; } }
        public BulkMaterialType BODmaterial { get { return m_BODmaterial; } set { m_BODmaterial = value; } }

        [CommandProperty(AccessLevel.GameMaster)]
        public int BribeCount { get { return m_BribeCount; } set { m_BribeCount = value; } }
        public Serial BribeDeed { get { return m_BribeDeed; } set { m_BribeDeed = value; } }
        public int BribeAmount { get { return m_BribeAmount; } set { m_BribeAmount = value; } }

        [CommandProperty(AccessLevel.GameMaster)]
        public DateTime GetBribeReleaseTime { get { if (m_BribeRelease == null) { return DateTime.MinValue; } return m_BribeRelease; } }

        public override bool CanTeach { get { return NpcConfig.NpcCanTeachEnabled; } }
        public override bool BardImmune { get { return NpcConfig.NpcBardImmuneEnabled; } }
        public override bool PlayerRangeSensitive { get { return NpcConfig.NpcPlayerRangeSensitiveEnabled; } }
        ...
        public override bool IsInvulnerable { get { return NpcConfig.NpcIsInvulnerableEnabled; } }
        ...
        public virtual DateTime GetNextBulkOrder74(Mobile from)
        {
        ...
        private class BulkOrderInfoEntry : ContextMenuEntry
        {
            ...
            public override void OnClick()
            {
                ...
                if (m_Vendor.SupportsBulkOrders(m_From))
                {
                    ...
                    {
                        int totalSeconds = (int)m_Vendor.GetNextBulkOrder74(m_From).Subtract(DateTime.UtcNow).TotalSeconds;
                        int totalHours = (totalSeconds + 3599) / 3600;
                        int totalMinutes = (totalSeconds + 59) / 60;

                        if (m_Vendor.GetNextBulkOrder74(m_From) < DateTime.UtcNow || m_From.AccessLevel > AccessLevel.Player)
                        {
        ...
        private class BribeEntry : ContextMenuEntry
        {
            ...
            public override void OnClick()
            {
                if (m_Vendor.GetBribeReleaseTime > DateTime.UtcNow)
                {
		...
		public virtual void InitOutfit()
		{
            ...
            if (Body == 0x191)
            {
                if (MainConfig.MainItemIDEnabled || T2AConfig.T2AItemIDEnabled)
                {
                    ...
                    {
                        case 0:
                            ShortPants sp = new ShortPants(GetRandomHue());
                            sp.Identified = true;
                            AddItem(sp);
                            break;
                        case 1:
                        case 2:
                            Kilt kt = new Kilt(GetRandomHue());
                            kt.Identified = true;
                            AddItem(kt);
                            break;
                        case 3:
                        case 4:
                        case 5:
                            Skirt st = new Skirt(GetRandomHue());
                            st.Identified = true;
                            AddItem(st);
                            break;
                    }
            ...
            else
            {
                if (MainConfig.MainItemIDEnabled || T2AConfig.T2AItemIDEnabled)
                {
                    ...
                    {
                        case 0:
                            LongPants lp = new LongPants(GetRandomHue());
                            lp.Identified = true;
                            AddItem(lp);
                            break;
                        case 1:
                            ShortPants sps = new ShortPants(GetRandomHue());
                            sps.Identified = true;
                            AddItem(sps);
                            break;
                    }
                ...
                if (m_BribeDeed != null && m_BribeAmount > 0)
                {
                    if (dropped.Serial == m_BribeDeed)
                    {
                        if (Banker.Withdraw(pm, m_BribeAmount))
                        {
                            ...
                            from.SendLocalizedMessage(1060398, m_BribeAmount.ToString());	// ~1_AMOUNT~ gold has been withdrawn from your bank box.	//2015-01-24 Added by Higoo
                            m_BribeAmount = 0;
                            m_BribeCount++;
                            if (m_BribeCount == BribeLimitCount)
                            {
                                m_BribeRelease = DateTime.UtcNow.Add(TimeSpan.FromHours(2));
                            }
                            return false;
                        }
                        else
		...
		public virtual bool OnBuyItems(Mobile buyer, List<BuyItemResponse> list)
		{
			...
			bought = (buyer.IsStaff());

			cont = buyer.Backpack;

            if (Core.TOL)
            {
                if (!bought && totalCost >= 0 || AccountGold.Enabled)
                {
		...
		public override void AddCustomContextEntries(Mobile from, List<ContextMenuEntry> list)
		{
            ...
            {
                if (NpcConfig.NpcBODSystemEnabled)
                {
                ...
                if (NpcConfig.NpcBuySellContextEnabled)
                {
                    if (IsActiveSeller)
                    {
