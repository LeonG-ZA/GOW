...
namespace Server.Mobiles
{
	...
	public abstract class BaseAI
	{
		...
		{
			if (from.Alive && m_Mobile.Controlled && from.InRange(m_Mobile, 12))
			{
		...
		public virtual MoveResult DoMoveImpl(Direction d)
		{
			...
			else if (!m_Mobile.Move(d))
			{
                ...
                if (this is MageAI && m_Mobile.ControlOrder == OrderType.Come)
                {
                    ...
                    {
                        if (m_Mobile.Target is TeleportSpell.TeleportSpellTarget)
                        {
                            Think();
		...
		public virtual bool AcquireFocusMob(int iRange, FightMode acqType, bool bPlayerOnly, bool bFacFriend, bool bFacFoe)
		{
			...
			if (map != null)
			{
				...
				foreach (Mobile m in eable)
				{
					...
					//if (TransformationSpell.UnderTransformation(m, typeof(EtherealVoyageSpell)))
					//{
					//	continue;
					//}
					...
					if (acqType == FightMode.Aggressor || acqType == FightMode.Evil || (m is BaseCreature) && ((BaseCreature)m).Summoned || acqType == FightMode.Good)
					{
