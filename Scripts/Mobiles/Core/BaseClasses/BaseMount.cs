...
namespace Server.Mobiles
{
    ...
    public abstract class BaseMount : BaseCreature, IMount
    {
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            if (!CheckMountAllowed(this, true))
                return;//in Question

            if (this is LesserHiryu)
            {
                if (from.Skills[SkillName.Bushido].Value < 90.0 && from.Skills[SkillName.AnimalTaming].Value <= 90.0)
                {
                    from.SendLocalizedMessage(1063021); // That creature can only be ridden by Bushido masters and master Tamers.
                    return;
                }
            }
    ...
    {
        public override bool CanInsureItem { get { return false; } }
