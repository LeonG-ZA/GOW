...
namespace Server.Mobiles
{
	...
	public class BaseCreature : Mobile, IHonorTarget
	{
		...
		public Mobile m_ControlMaster; // My master
        ...
        public virtual bool CanBeBlackRock { get { return true; } }
        ...
        [CommandProperty(AccessLevel.GameMaster)]
        public bool HasBeenStolen { get { return m_HasBeenStolen; } set { m_HasBeenStolen = value; } }
		...
		#region Bonding
        public const bool BondingEnabled = NpcConfig.NpcBondingEnabled;

		public virtual bool IsBondable { get { return (BondingEnabled && !Summoned); } }
        public virtual TimeSpan BondingDelay { get { return NpcConfig.NpcBondingDelay; } }
        public virtual TimeSpan BondingAbandonDelay { get { return NpcConfig.NpcBondingAbandonDelay; } }
		...
		public virtual void BreathDealDamage(Mobile target)
		{
			if (!Evasion.CheckSpellEvasion(target))
			{
				...
				{
					ItemAttributes.Damage(target, this, BreathComputeDamage(), physDamage, fireDamage, coldDamage, poisDamage, nrgyDamage);
				}
			}
		}
        ...
        */
        ...
        #region Pet Friends
        public const int MaxOwners = NpcConfig.NpcPetMaxOwners;
		...
		public override string ApplyNameSuffix(string suffix)
		{
            ...
            else if (IsBlackRock)
            {
                ...
                {
                    suffix = String.Concat(suffix, " (BlackRock Infected)");
                }
            }
        ...
        public virtual bool ForceNotoriety { get { return false; } }
        ...
        {
            #region Talisman Might be duped from baseweapon just commented it out for now untell further testing
            /*
		...
		public virtual void OnCarve(Mobile from, Corpse corpse, Item with)
		{
            ...
            if ((feathers == 0 && wool == 0 && meat == 0 && hides == 0 && scales == 0 && dragonblood == 0 && fur == 0 && pelt == 0 && scoots == 0) || Summoned || IsBonded || corpse.Animated)
			{
				if (corpse.Animated)
				{
			...
			else
			{
                ...
                if (pelt != 0)
                {
                    if (PeltType == PeltType.Orange)
                        corpse.AddCarvedItem(new OrangeTigerPelt(pelt), from);
                    else if (PeltType == PeltType.White)
                        corpse.AddCarvedItem(new WhiteTigerPelt(pelt), from);
                    else if (PeltType == PeltType.Black)
                        corpse.AddCarvedItem(new BlackTigerPelt(pelt), from);
                ...
                if (scoots != 0)
                {
                    corpse.AddCarvedItem(new DragonTurtleScute(scoots), from);
		...
		public override void Serialize(GenericWriter writer)
		{
            ...
            writer.Write(27); // version old 25
		...
		{
			/* Cooked */
			typeof(SliceOfBacon), typeof(CookedBird), typeof(Sausage), typeof(Ham), typeof(Ribs), typeof(LambLeg), typeof(ChickenLeg),
			/* Uncooked */
		...
		public virtual bool CheckFeed(Mobile from, Item dropped)
		{
			if (!IsDeadPet && Controlled && (ControlMaster == from || IsPetFriend(from)) &&
                (dropped is BaseFood || dropped is Gold || dropped is BaseCookableFood || dropped is Head || dropped is LeftArm ||
                 ...
                 dropped is GoldIngot || dropped is AgapiteIngot || dropped is VeriteIngot || dropped is ValoriteIngot || dropped is IPetBooster))
			{
				Item f = dropped;
		...
		public void ChangeAIType(AIType NewAI)
		{
			...
			switch (NewAI)
			{
                ...
                case AIType.AI_Scalis:
                    ...
                    break;
                case AIType.AI_AdvancedArcherAI:
                    m_AI = new AdvancedArcherAI(this);
                    break;
        ...
        public virtual int Scoots { get { return 0; } }

        public virtual int Fur { get { return 0; } }
        public virtual FurType FurType { get { return FurType.Green; } }

        public virtual int Pelt { get { return 0; } }
        public virtual PeltType PeltType { get { return PeltType.Orange; } }
		#endregion
		...
		*/
		...
		public override void OnMovement(Mobile m, Point3D oldLocation)
		{
            ...
            else if (ReacquireOnMovement || m_Paragon)
			{
				ForceReacquire();
			}
        ...
        public void PackMysticScroll(int index)
        {
            ...
            PackItem(Loot.Construct(Loot.MysticismScrollTypes, index));
        }
		...
		public override bool OnBeforeDeath()
		{
            ...
            if (!Summoned && !NoKillAwards && !IsBonded && TreasureMapLevel >= 1)
            {
                ...
                else if (m_BlackRock && BlackRock.ChestChance > Utility.RandomDouble())
                {
                    PackItem(new BlackRockChest(this.Name, treasureLevel));
                }
            ...
            if (!NoKillAwards && !Summoned && !m_HasGeneratedLoot && Region.IsPartOf("Doom") && !(this is DarkGuardian))
            {
		...
		public virtual void OnKilledBy(Mobile mob)
		{
            ...
            if (GivesSAArtifact)
			{
                if (StygianAbyss.SACheckArtifactChance(mob, this))
                {
                    StygianAbyss.GiveSAArtifact(mob);
                }
			}

            if (LogConfig.LogPvMLoggingEnabled)
            {
                if (mob is PlayerMobile)
                {
                    ...
                    path = Path.Combine(path, String.Format("Attacker.{0}.log", pm.Name));
		...
		/* To save on cpu usage, GOW creatures only reacquire creatures under the following circumstances:
        ...
        */
		...
		public static bool Summon(BaseCreature creature, bool controlled, Mobile caster, Point3D p, int sound, TimeSpan duration)
		{
            ...
            //creature.SetHits((int)Math.Floor(creature.HitsMax * (1 + ArcaneEmpowermentSpell.GetSummonHitsBonus(caster) / 100.0)));
        ...
        #region Rummaging
        public static bool EnableRummaging = NpcConfig.NpcEnableRummaging;

        public const double ChanceToRummage = NpcConfig.NpcChanceToRummage;

        public const double MinutesToNextRummageMin = NpcConfig.NpcMinutesToNextRummageMin;
        public const double MinutesToNextRummageMax = NpcConfig.NpcMinutesToNextRummageMax;

        public const double MinutesToNextChanceMin = NpcConfig.NpcMinutesToNextChanceMin;
        public const double MinutesToNextChanceMax = NpcConfig.NpcMinutesToNextChanceMax;
		...
		public static void Scale(BaseCreature bc, int scalar)
		{
			...
			toScale = bc.RawStr;
			bc.RawStr = ItemAttributes.Scale(toScale, scalar);

			toScale = bc.HitsMaxSeed;

			if (toScale > 0)
			{
				bc.HitsMaxSeed = ItemAttributes.Scale(toScale, scalar);
			}
		...
		public virtual void AuraDamage()
		{
			...
			{
				ItemAttributes.Damage(m, this, AuraBaseDamage, AuraPhysicalDamage, AuraFireDamage, AuraColdDamage, AuraPoisonDamage, AuraEnergyDamage, AuraChaosDamage);
				AuraEffect(m);
			}
		}
	...
	public class LoyaltyTimer : Timer
	{
		...
		protected override void OnTick()
		{
            bool hasHourElapsed = (DateTime.UtcNow >= m_NextHourlyCheck);
