...
namespace Server.SkillHandlers
{
	public class Stealing
	{
        ...
        private static Type[] m_SpecialDrops = new Type[]
			{
				typeof( BalmOfProtection ),		typeof( BalmOfStrength ),
				typeof( BalmOfSwiftness ),		typeof( BalmOfWisdom ),
				typeof( GemOfSalvation ),		typeof( LifeShieldLotion ),
				typeof( ManaDraught ),			typeof( SeedOfLife ),
				typeof( StoneSkinLotion )
			};

        public static double SpecialDropChance(Mobile m, BaseCreature monster)
        {
            if (monster.ControlMaster != null)
                return 0.0;

            if (m.Skills[SkillName.Stealing].Value < 100.0)
                return 0.0;
            ...
            chance += (m.Skills[SkillName.Stealing].Value - 100.0) / 200.0;
            chance += Math.Max(0.0, (monster.Fame - 10000) / 100000.0);
		...
		private class StealingTarget : Target
		{
			...
			protected override void OnTarget(Mobile from, object target)
			{
				...
				else if (target is Mobile)
				{
                    ...
                    {
                        Pub57.ThieveItems.StealingHandler.HandleSteal(target as BaseCreature, from as PlayerMobile);
                    }
                     */
