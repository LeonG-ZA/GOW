...
namespace Server.SkillHandlers
{
	public class AnimalTaming
	{
		...
		private class TamingTarget : Target
		{
					...
					if (targeted is BaseCreature)
					{
						...
						else if (CheckMastery(from, creature) || from.Skills[SkillName.AnimalTaming].Value >= creature.MinTameSkill)
						{
							...
							else if (creature.CanAngerOnTame && 0.95 >= Utility.RandomDouble())
							{
								...
								if (from is PlayerMobile &&
									!(((PlayerMobile)from).HonorActive ||
									  TransformationSpell.UnderTransformation(from, typeof(EtherealVoyageSpell))))
								{
							...
							else
							{
								...
								new TamingTimer(from, creature, Utility.Random(3, 2)).Start();
			...
			private class TamingTimer : Timer
			{
                ...
                public TamingTimer(Mobile tamer, BaseCreature creature, int count)
					...
					else
					{
						...
						if (CheckMastery(m_Tamer, m_Creature) || alreadyOwned || m_Tamer.CheckTargetSkill(SkillName.AnimalTaming, m_Creature, minSkill - 25.0, minSkill + 25.0))
						{
							...
							else
							{
                                ...
                                PlayerMobile pm = m_Tamer as PlayerMobile;

                                //for TamingObjective Quest Check                                
                                if (ExtendedQuestHelper.CheckTamedObject(pm, m_Creature))
                                    m_Tamer.SendMessage("");
							}
