        ...
        private class RemoveTrapTarget : Target
        {
            ...
            protected override void OnTarget(Mobile from, object targeted)
            {
                ...
                else if (targeted is TrapableContainer)
                {
                    ...
                    /*
                else if (targeted is GoblinTrapArmed)
                {
                    GoblinTrapArmed trap = (GoblinTrapArmed)targeted;

                    if ((Core.ML) || (from.CheckTargetSkill(SkillName.RemoveTrap, trap, 80.0, 100.0)))
                    {
                        from.SendLocalizedMessage(502377); // You successfully render the trap harmless
                        GoblinTrapUnArmed trap2 = new GoblinTrapUnArmed();
                        trap2.MoveToWorld(trap.Location, trap.Map);
                        trap.Delete();
                    }
                    else
                    {
                        from.SendLocalizedMessage(502371); // You fail to disarm the trap... but you don't set it off
                    }
                }
                     */
