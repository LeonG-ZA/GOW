...
if (o is BaseWeapon || o is BaseArmor || o is BaseJewel || o is BaseHat)
{
    ...
    else if (MainConfig.MainItemIDEnabled)
    {
        if (from.CheckTargetSkill(SkillName.ItemID, o, 0, 100))
        {
            if (o is BaseWeapon)
                ...
                ((BaseArmor)o).Identified = true;
            from.LocalOverheadMessage(MessageType.Regular, 2304, false, "You do identify the magical Properties");
        }
    }
    else if (from.Skills[SkillName.ItemID].Base >= 100.0)
    {
        if (o is BaseWeapon)
        {
    ...
    {
        if (MainConfig.MainItemIDEnabled)
        {
