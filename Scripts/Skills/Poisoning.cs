...
namespace Server.SkillHandlers
{
    public class Poisoning
    {
        ...
        {
            public PoisoningTargetPoison() : base(2, false, TargetFlags.None)
            {
            }
            ...
            protected override void OnTarget(Mobile from, object targeted)
            {
                if (targeted is BasePoisonPotion)
                {
                    ...
                    from.Target = new PoisonPotionTarget((BasePoisonPotion)targeted);
                }
            ...
            private class PoisonPotionTarget : Target
            {
                private readonly BasePoisonPotion m_Potion;
                public PoisonPotionTarget(BasePoisonPotion potion) : base(2, false, TargetFlags.None)
                {
                ...
                protected override void OnTarget(Mobile from, object targeted)
                {
                    ...
                    if (targeted is BaseFood || targeted is FukiyaDart || targeted is Shuriken)
                    {
                        startTimer = true;
                    }
                    ...
                    if (startTimer)
                    {
                        new PotionTimer(from, (Item)targeted, this.m_Potion).Start();
                ...
                private class PotionTimer : Timer
                {
                    ...
                    public PotionTimer(Mobile from, Item target, BasePoisonPotion potion) : base(TimeSpan.FromSeconds(2.0))
                    {
                        this.m_From = from;
                    ...
                    protected override void OnTick()
                    {
                        ...
                        {
                            if (this.m_Target is BaseFood)
                            {
                                ((BaseFood)this.m_Target).Poison = this.m_Poison;
                            }
                            ...
                            else if (this.m_Target is FukiyaDart)
                            {
                                ((FukiyaDart)this.m_Target).Poison = this.m_Poison;
                                ((FukiyaDart)this.m_Target).PoisonCharges = Math.Min(18 - (this.m_Poison.RealLevel * 2), ((FukiyaDart)this.m_Target).UsesRemaining);
                            }
