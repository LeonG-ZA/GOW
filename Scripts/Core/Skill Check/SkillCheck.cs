...
namespace Server.Misc
{
    ...
    {
        private static readonly bool AntiMacroCode = CharConfig.CharAntiMacroCode;		//Change this to false to disable anti-macro code

        public static TimeSpan AntiMacroExpire = CharConfig.CharAntiMacroExpire; //How long do we remember targets/locations?
        public const int Allowance = CharConfig.CharAllowance;	//How many times may we use the same location/target for gain
        private const int LocationSize = CharConfig.CharLocationSize; //The size of eeach location, make this smaller so players dont have to move as far
        ...
        public static bool CheckSkill(Mobile from, Skill skill, object amObj, double chance)
        {
            ...
            if (from.Alive && ((gc >= Utility.RandomDouble() && AllowGain(from, skill, amObj)) || skill.Base < 10.0 || (GuaranteedGainSystem.Enabled && GuaranteedGainSystem.ForceSkillGain(from, skill))))
                Gain(from, skill);
        ...
        public static void Gain(Mobile from, Skill skill)
        {
            ...
            {
                bool gainAllowed = (RateOverTime.Enabled ? RateOverTime.SkillGainAllowed(from, skill) : true);
                ...
                if (gainAllowed && (skills.Total / skills.Cap) >= Utility.RandomDouble())//( skills.Total >= skills.Cap )
                {
                ...
                if (from is PlayerMobile)
                    if (QuestHelper.EnhancedSkill((PlayerMobile)from, skill))
                ...
                if (!from.Player || gainAllowed && (skills.Total + toGain) <= skills.Cap)
                {
                    skill.BaseFixedPoint += toGain;

                    if (GuaranteedGainSystem.Enabled)
            ...
            if (skill.Lock == SkillLock.Up)
            {
                ...
                if (GuaranteedGainSystem.Enabled && GuaranteedGainSystem.ForceStatGain(from))
                {
                ...
                case Stat.Str:
                    {
                        ...
                        else if ((from.LastStrGain + m_StatGainDelay) >= DateTime.UtcNow || (RateOverTime.Enabled && !RateOverTime.StatGainAllowed(from)))
                            return;
                ...
                case Stat.Dex:
                    {
                        ...
                        else if ((from.LastDexGain + m_StatGainDelay) >= DateTime.UtcNow || (RateOverTime.Enabled && !RateOverTime.StatGainAllowed(from)))
                            return;
                ...
                case Stat.Int:
                    {
                        ...
                        else if ((from.LastIntGain + m_StatGainDelay) >= DateTime.UtcNow || (RateOverTime.Enabled && !RateOverTime.StatGainAllowed(from)))
                            return;
