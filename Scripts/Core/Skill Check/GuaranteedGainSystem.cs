...
namespace Server.Engines
{
    public class GuaranteedGainSystem : Timer
	{
		...
		public static bool Enabled = CharConfig.CharGGSEnabled;

        public static TimeSpan ResetTime = CharConfig.CharGGSResetTime;
        public static string SavePath = CharConfig.CharGGSSavePath;
        public static string SaveFile = CharConfig.CharGGSSaveFile;
		...
		public static bool ForceSkillGain( Mobile from, Skill skill )
		{
			if ( from.Player )
			{
				...
				if ( DateTime.UtcNow - skillInfo.LastGainTime < TimeSpan.FromMinutes( table[ index > 23 ? 23 : index ] ) )
					return false;
		...
		public static bool ForceStatGain( Mobile from )
		{
			if ( from.Player )
			{
				...
				if ( mobileInfo.StatGainsCount < 10 && DateTime.UtcNow - mobileInfo.LastStatGainTime > TimeSpan.FromMinutes( 15 ) )
					return true;
			}
		...
		public static void RegisterStatGain( Mobile from )
		{
			if ( from.Player )
			{
				...
				mobileInfo.LastStatGainTime = DateTime.UtcNow;
