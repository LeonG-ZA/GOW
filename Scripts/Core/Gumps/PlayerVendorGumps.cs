...
namespace Server.Gumps
{
	public class PlayerVendorBuyGump : Gump
	{
			...
			: base( 100, 200 )
		{
			...
			AddLabel( 125, 95, 0, vi.Price.ToString( "N0" ) );
	...
	public class NewPlayerVendorOwnerGump : Gump
	{
		...
		public override void OnResponse( NetState sender, RelayInfo info )
		{
			...
			switch ( info.ButtonID )
			{
				...
				case 5: // Rename Vendor
					{
						m_Vendor.Rename( from );

						break;
					}
				case 6: // Open Paperdoll
					{
			...
			else if ( info.ButtonID == 1 && info.Switches.Length > 0 )
			{
				...
				if ( cat < Categories.Length && cat >= 0 )
				{
					if ( ent < Categories[cat].Entries.Length && ent >= 0 )
					{
						...
						item = m_Vendor.GetEquippedItems().FirstOrDefault(
							( checkItem ) => Categories[cat].Entries.Any( ( entry ) => entry.Type == checkItem.GetType() ) );
				...
				else
				{
					...
					if ( cat < 100 )
					{
						if ( cat < Categories.Length && cat >= 0 )
						{
							var item = m_Vendor.GetEquippedItems().FirstOrDefault(
								( checkItem ) => Categories[cat].Entries.Any( entry => entry.Type == checkItem.GetType() ) );
