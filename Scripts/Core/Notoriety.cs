...
namespace Server.Misc
{
    public class NotorietyHandlers
    {
        ...
        public static bool Mobile_AllowBeneficial(Mobile from, Mobile target)
        {
            ...
            Engines.ConPVP.SafeZone sz = from.Region.GetRegion(typeof(SafeZone)) as SafeZone;
        ...
        public static int MobileNotoriety(Mobile source, Mobile target)
        {
            ...
            if (target is BaseCreature && !(target is BaseEscort))
            {
            ...
            if (target.Kills >= 5 || (target.Body.IsMonster && IsSummoned(target as BaseCreature) && !(target is BaseFamiliar) && !(target is LesserFey) && !(target is Golem)) || (target is BaseCreature && (((BaseCreature)target).AlwaysMurderer || ((BaseCreature)target).IsAnimatedDead)))
                return Notoriety.Murderer;
            ...
            else if (source.Player && target is BaseEscort)
                return Notoriety.Innocent;
            ...
            {
                if (!target.Body.IsHuman && !target.Body.IsGhost && !IsPet(target as BaseCreature) && !(target is PlayerMobile) || !Core.ML && !target.CanBeginAction(typeof(PolymorphSpell)))
                    return Notoriety.CanBeAttacked;
            }
             */
            ...
            if (!(target is BaseCreature && ((BaseCreature)target).InitialInnocent))
            {
                if (!target.Body.IsHuman && !target.Body.IsGhost && !IsPet(target as BaseCreature) && !TransformationSpell.UnderTransformation(target) && !AnimalForm.UnderTransformation(target) && target.Race != Race.Elf)
                    return Notoriety.CanBeAttacked;
            }
