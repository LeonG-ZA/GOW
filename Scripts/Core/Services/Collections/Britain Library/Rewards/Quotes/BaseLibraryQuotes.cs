namespace Server.Items
{
    public abstract class BaseLibraryQuotes : MonsterStatuette
    {
        ...
        {
            if (TurnedOn && IsLockedDown && (!m.Hidden || true/*m.AccessLevel == AccessLevel.Player*/ ) && m.InRange(m, 2) && !m.InRange(oldLocation, 2))
            {
                PublicOverheadMessage(Network.MessageType.Regular, 0, Utility.RandomMinMax(MsgMin, MsgMax));
