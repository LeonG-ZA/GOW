...
{
    public class BritainLibraryMageCollection : BritainLibraryCollection
    {
        public override int Section { get { return 1073442; } } // Wizard's Compendium Section Donation Representative.
        ...
        private static readonly IRewardEntry[] RewardList = new IRewardEntry[]
            {
				...
				// page 6
				new RewardEntry( typeof( WizardsCrystalReadingGlasses ), 800000, 1073374, 12216, 557 ),
            };
        ...
        private static readonly DonationEntry[] DonationList = new DonationEntry[]
            {
                ...
                new DonationEntry( typeof( Spellbook ), 10, 3010065, 3834, 0 ),
                new DonationEntry( typeof( BlackPearl ), 1, 1044353, 3962, 0 ),

				// page 2
				new DonationEntry( typeof( Bloodmoss ), 1, 1044354, 3963, 0 ),
                new DonationEntry( typeof( Garlic ), 1, 1044355, 3972, 0 ),
                new DonationEntry( typeof( Ginseng ), 1, 1044356, 3973, 0 ),
                new DonationEntry( typeof( MandrakeRoot ), 1, 1044357, 3974, 0 ),
                new DonationEntry( typeof( Nightshade ), 1, 1044358, 3976, 0 ),
                new DonationEntry( typeof( SulfurousAsh ), 1, 1044359, 3980, 0 ),
				
				// page 3
				new DonationEntry( typeof( SpidersSilk ), 1, 1015007, 3981, 0 ),
        ...
        {
            MageSectionRepresentative npc = new MageSectionRepresentative();
            npc.Controller = this;
            Representative = npc;
        ...
        [Constructable]
        public BritainLibraryMageCollection()
        {
            Visible = false;
        }

        public BritainLibraryMageCollection(Serial serial)
            : base(serial)
        {
        }
