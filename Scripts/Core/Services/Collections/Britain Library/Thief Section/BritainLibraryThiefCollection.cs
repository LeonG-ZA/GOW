...
{
	public class BritainLibraryThiefCollection : BritainLibraryCollection
	{
		...
		private static readonly IRewardEntry[] RewardList = new IRewardEntry[]
			{
				...
				// page 6
				new RewardEntry( typeof( PoisonedReadingGlasses ), 800000, 1073376, 12216, 275 ),
			};
		...
		private static readonly DonationEntry[] DonationList = new DonationEntry[]
			{
				...
				new DonationEntry( typeof( Lockpick ), 1, 1025372, 5371, 0 ),
				new DonationEntry( typeof( LesserPoisonPotion ), 1, 1044548, 3850, 0 ),
				new DonationEntry( typeof( DeadlyPoisonPotion ), 5, 1044551, 3850, 0 ),
				
				// page 2
				new DonationEntry( typeof( ClothNinjaHood ), 11, 1074911, 10202, 0 ),
				new DonationEntry( typeof( ClothNinjaHood ), 11, 1074911, 10127, 0 ),
				new DonationEntry( typeof( LeatherNinjaHood ), 16, 1074912, 10201, 0 ),
				new DonationEntry( typeof( LeatherNinjaHood ), 16, 1074912, 10126, 0 ),
		...
		{
			ThiefSectionRepresentative npc = new ThiefSectionRepresentative();
			npc.Controller = this;
		...
		[Constructable]
		public BritainLibraryThiefCollection()
		{
			Visible = false;
		}

		public BritainLibraryThiefCollection( Serial serial )
			: base( serial )
		{
		}
