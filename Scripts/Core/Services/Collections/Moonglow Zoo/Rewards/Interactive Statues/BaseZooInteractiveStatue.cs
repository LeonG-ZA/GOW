namespace Server.Items
{
    public abstract class BaseZooInteractiveStatue : MonsterStatuette
    {
        ...
        {
            if (TurnedOn && IsLockedDown && (!m.Hidden || m.AccessLevel == AccessLevel.Player) && m.InRange(m, 2) && !m.InRange(oldLocation, 2))
            {
                PublicOverheadMessage(Network.MessageType.Regular, 0, Utility.RandomMinMax(1073207, 1073218));
