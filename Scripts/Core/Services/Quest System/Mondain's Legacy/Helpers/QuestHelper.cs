...
namespace Server.Engines.Quests
{
    ...
    { 
            ...
            for (int i = player.Quests.Count - 1; i >= 0; i --)
            {
                ...
                for (int j = quest.Objectives.Count - 1; j >= 0; j --)
                {
                    ...
                    else if (objective is GuildCraftObjective)
                    {
                        ...
                        if (craft.Update(item))
                        {
                            if (quest.Completed)
                                quest.OnCompleted();
                            else if (craft.Completed)
                                player.PlaySound(quest.UpdateSound);
        ...
        public static bool HasQuestNew<T>(PlayerMobile from) where T : BaseQuest
        {
            return GetQuestNew<T>(from) != null;
        }

        public static T GetQuestNew<T>(PlayerMobile from) where T : BaseQuest
        {
            for (int i = from.Quests.Count - 1; i >= 0; i--)
            {
                BaseQuest quest = from.Quests[i];
