...
namespace Server.Factions
{
    public class FactionImbueGump : FactionGump
    {
        ...
        public FactionImbueGump(int quality, Item item, Mobile from, CraftSystem craftSystem, IUsesRemaining tool, object notice, int availableSilver, Faction faction, FactionItemDefinition def)
            : base(100, 200)
        { 
            this.m_Item = item;
            this.m_Mobile = from;
        ...
        public override void OnResponse(NetState sender, RelayInfo info)
        {
            if (info.ButtonID == 1)
            {
                ...
                if (pack != null && this.m_Item.IsChildOf(pack))
                {
                    if (pack.ConsumeTotal(typeof(Silver), this.m_Definition.SilverCost))
                    {
                        ...
                        if (this.m_Item is BaseSpellScroll)
                            hue = 0;
            ...
            if (this.m_Tool != null && !((Item)m_Tool).Deleted && this.m_Tool.UsesRemaining > 0)
