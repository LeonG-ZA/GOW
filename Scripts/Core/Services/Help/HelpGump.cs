...
namespace Server.Engines.Help
{
    ...
    public class HelpGump : Gump
    {
        ...
        private static void EventSink_HelpRequest(HelpRequestEventArgs e)
        {
            ...
            if (FeaturesConfig.FeatAutoStaffTeamEnabled)  //If Enabled, Begin The New Help Gump Process
            {
                if (((pm.LastTimePaged + CanHelpAgain) <= DateTime.UtcNow))
                {
                    ...
                    sm.Location = new Point3D(e.Mobile.X, e.Mobile.Y + 5, e.Mobile.Z);
                    sm.MoveToWorld(sm.Location, e.Mobile.Map);
                ...
                e.Mobile.SendMessage("You May Only Page Staff Once Every Thirty Minutes. If You Need Assistance Now, Please Visit Our Website At: http://unforgetable6910.wix.com/distantland");
                return;
            }
            ...
            {
                // Temporary code
                ConsoleLog.Write("Help request with gumps == null, from={0}, state={1}", from, state);
                return;
            }
        ...
        {
            get { return TimeSpan.FromMinutes(30); } //Player Wait Time Is Set To 30 Minutes
            ...
            : base(0, 0)
        {
            ...
            if (PromotionalSystem.HasRequest(from))
            {
                AddLabel(110, 393, 0x672, "Promotional Items unavailable. Try again in 30 secs.");
            }
            else
            {
                if (isYoung)
                {
                    AddHtmlLocalized(110, 405, 450, 60, 1062870, true, true); // <u>Promotional Code:</u> Select this option if you have a promotional code that you wish to redeem with this character or account.
                    AddButton(80, 406, 5540, 5541, 11, GumpButtonType.Reply, 0);
                }
                else
                {
                    AddHtmlLocalized(110, 339, 450, 60, 1062870, true, true); // <u>Promotional Code:</u> Select this option if you have a promotional code that you wish to redeem with this character or account.
