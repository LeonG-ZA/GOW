    ...
    public class PrimeSpawner : Item
    {
        ...
        private class WarnTimer : Timer
        {
            ...
            protected override void OnTick()
            {
                try
                {
                    ErrorLog.WriteLine("Warning: {0} bad spawns detected, logged: 'PremiumBadspawn.log'", I_List.Count);
                    using (StreamWriter op = new StreamWriter("Logs/PremiumBadspawn.log", true))
                    {
