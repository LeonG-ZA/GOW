...
namespace Server.Engines.Harvest
{
    public class HarvestTarget : Target
    {
        ...
        protected override void OnTarget(Mobile from, object targeted)
        {
            ...
            if (m_System is Lumberjacking)
            {
                ...
                else if (targeted is IAxe && m_Tool is BaseAxe)
                {
                    ...
                    if (!item.IsChildOf(from.Backpack))
                    {
                        ...
                        from.SendMessage("I'm being used in lumberjacking fix me."); 
                    }
                ...
                else if (m_Tool.Parent != from)
                {
                    from.SendLocalizedMessage(500487); // The axe must be equipped for any serious wood chopping.
                }
                else
                {
                    m_System.StartHarvesting(from, this.m_Tool, targeted);
                }
            }
            else if (m_System is Mining)
            {
            ...
            else if (m_System is Lumberjacking && targeted is LandTarget)
            {
                from.SendLocalizedMessage(500489); // You can't use an axe on that.
            }
            else if (m_System is Lumberjacking && m_Tool.Parent != from)
            {
                from.SendLocalizedMessage(500487); // The axe must be equipped for any serious wood chopping.
            }
            else
