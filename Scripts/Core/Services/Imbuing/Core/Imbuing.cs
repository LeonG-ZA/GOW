...
namespace Server.SkillHandlers
{
    ...
    public class Imbuing
    {
        ...
        public static bool CanImbueItem(Mobile from, Item item)
        {
            ...
            else if (item is BaseWeapon && Spells.Mysticism.EnchantSpell.UnderEffect(from))
            ...
            else if (IsInNonImbueList(item.GetType()) || item is ISetItem || item is ICollectionItem || !imbuable.CanImbue)
                from.SendLocalizedMessage(1079576); // You cannot imbue this item.
        ...
        {
            /*
            if (GetValueForMod(item, mod) == value)
            {
                from.SendMessage("You decide not to duplicate that intensity on that property.");
                return false;
            }
             */
            ...
            else
            {
                ...
                if (success >= Utility.RandomDouble() || mod < 0 || mod > 180)
                {
                    ...
                    if (i is BaseWeapon)
                    {
                        ...
                        if (prop is AosWeaponAttribute)
                        {
                            wep.WeaponAttributes.Balanced = 1;
                        }
                        if (prop is string)
                        {
                            ...
                            if (p == "BalancedWeapon" && wep is BaseRanged)
                                wep.WeaponAttributes.Balanced = value;
                            else if (p == "WeaponVelocity" && wep is BaseRanged)
                                wep.WeaponAttributes.Velocity = value;
                        }
                    }
                    else if (itemRef == 3)
                    {
                    ...
                    else if (i is BaseHat)
                    {
                        ...
                        if (prop is SAAbsorptionAttribute)
                            hat.AbsorptionAttributes[(SAAbsorptionAttribute)prop] = value;
        ...
        public static bool UnravelItem(Mobile from, Item item, bool message = true)
        {
            ...
            if (weight > 0)
            {
                ...
                if (weight >= (480 - bonus))
                {
                    if (from.Skills[SkillName.Imbuing].Base >= 95.0)
                    {
                        ...
                        {
                            item.Delete();
                            from.AddToBackpack(new EnchantedEssence(unravelQTY - Utility.Random(0, 3)));
                        }
                        else
                        {
                ...
                else if (weight > (200 - bonus) && weight < (480 - bonus))
                {
                    if (from.Skills[SkillName.Imbuing].Base >= 45.0)
                    {
                        ...
                        {
                            item.Delete();
                            from.AddToBackpack(new EnchantedEssence(unravelQTY));
                        }
            ...
            if (weight > 0)
            {
                ...
                if (weight <= (200 - bonus))
                {
                    success = from.CheckSkill(SkillName.Imbuing, 0.0, 45.0);
                }
                ...
                {
                    if (from.Skills[SkillName.Imbuing].Base >= 45.0)
                        success = from.CheckSkill(SkillName.Imbuing, 45.0, 95.0);
                    else
                    {
                        if (message)
                            from.SendLocalizedMessage(1080434); // Your Imbuing skill is not high enough to magically unravel this item.
                        return false;
                                 ...
                                 // == Relic Fragment ==
                else if (weight >= (480 - bonus))
                {
                    if (from.Skills[SkillName.Imbuing].Base >= 45.0)
                        success = from.CheckSkill(SkillName.Imbuing, 95.0, 120.0);
                    else
                    {
                        if (message)
                            from.SendLocalizedMessage(1080434); // Your Imbuing skill is not high enough to magically unravel this item.
                ...
                if (!success)
                {
                    ...
                    if (0.10 > Utility.RandomDouble()) //10% to break
                        item.Delete();
                }
                else
                {
                    ...
                    {
                        item.Delete();
                        from.AddToBackpack(new MagicalResidue());
                    }

                    if (weight > (200 - bonus) && weight < (480 - bonus))
                    {
                        item.Delete();
                        from.AddToBackpack(new EnchantedEssence());
                    }

                    if (weight >= (480 - bonus))
                    {
                        item.Delete();
                        from.AddToBackpack(new RelicFragment());
                    }
        ...
        public static int GetTotalMods(Item itw, int mod = -1)
        {
            ...
            if (itw is BaseWeapon)
            {
                ...
                /*
                if (wep is BaseWeapon && !(prop is string))
                {
                    BaseWeapon ranged = ranged as BaseWeapon;

                    if (wep.Balanced && mod != 61)
                        total++;
            ...
            else if (itw is BaseHat)
            {
                ...
                foreach (int i in Enum.GetValues(typeof(SAAbsorptionAttribute)))
                {
                    ...
                    if (h.AbsorptionAttributes[attr] > 0)
                    {
                        if (!(prop is SAAbsorptionAttribute) || ((SAAbsorptionAttribute)prop) != attr)
                            total += 1;
                    }
        ...
        public static int GetTotalWeight(Item item, int mod = -1)
        {
            ...
            else if (item is BaseHat)
            {
                ...
                saAttrs = ((BaseHat)item).AbsorptionAttributes;
            }
            else if (item is BaseJewel)
            {
        ...
        public static bool CheckSoulForge(Mobile from, int range, bool message)
        {
            ...
            {
                if (!isQueenForge)
                    isQueenForge = (item.ItemID >= 0x4277 && item.ItemID <= 0x4286); // Queens SoulForge (+5% bonus & easier unravels)

                if (!isForge)
                    isForge = (item.ItemID >= 0x4263 && item.ItemID <= 0x4272); // Standard SoulForge
        ...
        public static void LoadImbuingDefinition()
        {
            ...
            m_Table[3] = new ImbuingDefinition(AosAttribute.RegenHits, 1075627, 100, typeof(EnchantedEssence), typeof(Tourmaline), typeof(SeedRenewal), 2, 1, 1111994);
            m_Table[4] = new ImbuingDefinition(AosAttribute.RegenStam, 1079411, 100, typeof(EnchantedEssence), typeof(Diamond), typeof(SeedRenewal), 3, 1, 1112043);
            m_Table[5] = new ImbuingDefinition(AosAttribute.RegenMana, 1079410, 100, typeof(EnchantedEssence), typeof(Sapphire), typeof(SeedRenewal), 2, 1, 1112003);
            ...
            m_Table[24] = new ImbuingDefinition(AosWeaponAttribute.LowerStatReq, 1079757, 100, typeof(EnchantedEssence), typeof(Amethyst), typeof(ElvenFletchings), 100, 10, 1111998);
            ...
            m_Table[60] = new ImbuingDefinition(AosWeaponAttribute.Velocity, 1080416, 130, typeof(RelicFragment), typeof(Tourmaline), typeof(EssenceDirection), 50, 2, 1112048);
            m_Table[61] = new ImbuingDefinition(AosWeaponAttribute.Balanced, 1072792, 150, typeof(RelicFragment), typeof(Amber), typeof(EssenceBalance), 1, 0, 1112047);
            ...
            m_Table[200] = new ImbuingDefinition(SAAbsorptionAttribute.BloodDrinker, 1017407, 140, null, null, null, 1, 1, 1152387);
            m_Table[201] = new ImbuingDefinition(SAAbsorptionAttribute.BattleLust, 1113710, 140, null, null, null, 1, 1, 1152385);
            ...
            m_Table[205] = new ImbuingDefinition(SAAbsorptionAttribute.SplinteringWeapon, 1154670, 140, null, null, null, 20, 1, 1152396);
            m_Table[206] = new ImbuingDefinition(AosWeaponAttribute.Balanced, 1154660, 140, null, null, null, 1, 1, 1152400);
        ...
        //public static int GetModForAttribute((string)attr)
        //{
            //switch (attr)
            //{
              // case AosWeaponAttribute.Balanced: return 61;
              // case AosWeaponAttribute.Velocity: return 60;
            //}
        ...
        public static int[] GetPropRange(SAAbsorptionAttribute attr)
        {
            ...
            {
                default:
                case SAAbsorptionAttribute.BloodDrinker: return new int[] { 1, 1 };
                case SAAbsorptionAttribute.BattleLust:
                ...
                case SAAbsorptionAttribute.SplinteringWeapon: return new int[] { 5, 30 };
            }
        }
