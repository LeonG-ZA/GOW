        ...
        public override void OnResponse(NetState sender, RelayInfo info)
        {
            if (info.ButtonID == 1)
            {
                ...
                if (MainConfig.MainItemIDEnabled || T2AConfig.T2AItemIDEnabled)
                {
                    ...
                    else if (item is Server.Items.BaseClothing)
                        ((Server.Items.BaseClothing)item).Identified = true;
                    else if (item is Server.Items.BaseJewel)
                        ((Server.Items.BaseJewel)item).Identified = true;
                }
                if (item != null)
                {
