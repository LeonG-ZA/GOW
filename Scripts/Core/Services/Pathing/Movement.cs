    ...
    public class MovementImpl : IMovementImpl
    {
        ...
        private bool Check(Map map, Mobile m, List<Item> items, List<Mobile> mobiles, int x, int y, int startTop, int startZ, bool canSwim, bool cantWalk, out int newZ)
        {
            ...
            for (int i = 0; i < tiles.Length; ++i)
            {
                ...
                if ((flags & ImpassableSurface) == TileFlag.Surface || (canSwim && (flags & TileFlag.Wet) != 0) || (m.Flying && (flags & TileFlag.NoDiagonal) != 0)) // Surface && !Impassable
                {
                    if (cantWalk && (flags & (TileFlag.Wet | TileFlag.NoDiagonal )) == 0)
                        continue;
            ...
            for (int i = 0; i < items.Count; ++i)
            {
                ...
                if (!item.Movable && ((flags & ImpassableSurface) == TileFlag.Surface || (m.CanSwim && (flags & TileFlag.Wet) != 0) || (m.Flying && (flags & TileFlag.NoDiagonal) != 0))) // Surface && !Impassable && !Movable
                {
                    if (cantWalk && (flags & (TileFlag.Wet | TileFlag.NoDiagonal ) ) == 0)
                        continue;
        ...
        private void GetStartZ(Mobile m, Map map, Point3D loc, List<Item> itemList, out int zLow, out int zTop)
        {
            ...
            for (int i = 0; i < staticTiles.Length; ++i)
            {
                ...
                if ((!isSet || calcTop >= zCenter) && ((id.Flags & TileFlag.Surface) != 0 || (m.CanSwim && (id.Flags & TileFlag.Wet) != 0) || (m.Flying && (id.Flags & TileFlag.NoDiagonal) != 0)) && loc.Z >= calcTop)
                {
                    if (m.CantWalk && (id.Flags & ( TileFlag.Wet | TileFlag.NoDiagonal ) ) == 0 )
                        continue;
