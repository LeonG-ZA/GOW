...
namespace Server.Engines.Craft
{
    ...
    public class Alter
    {
        ...
        {
            from.Target = new AlterTarget(craftSystem, contract);
        ...
        {
            from.Target = new AlterTarget(craftSystem, tool);
            from.SendLocalizedMessage(1094730); // Target the item to alter.
        }

        private class AlterTarget : Target
        {
            ...
            public AlterTarget(CraftSystem craftSystem, Item contract)
            ...
            //public AlterTarget(CraftSystem craftSystem, BaseTool tool)
            public AlterTarget(CraftSystem craftSystem, IUsesRemaining tool)
                : base(2, false, TargetFlags.None)
            {
                m_CraftSystem = craftSystem;
                m_Tool = tool;
            ...
            protected override void OnTarget(Mobile from, object targeted)
            {
                ...
                else if (Imbuing.CheckSoulForge(from, 2, false))
                {
                    // You must be near a soulforge to alter an item.
                    number = 1111867;
                }
                //else if (!Imbuing.CheckSoulForge.CheckQueen(from))
                //{
