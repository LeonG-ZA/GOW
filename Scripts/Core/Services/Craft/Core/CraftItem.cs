...
namespace Server.Engines.Craft
{
	...
	public interface ICraftable
	{
        ...
        int OnCraft(int quality, bool makersMark, Mobile from, CraftSystem craftSystem, Type typeRes, IUsesRemaining tool, CraftItem craftItem, int resHue);
	}
	...
	public class CraftItem
	{
		...
		private static readonly int[] m_HeatSources = new[]
		{
            ...
            0x9A63, 0x9A64, 
            0x9A65, 0x9A66, 
            0x9A67, 0x9A68,// Gold Marble Fireplace
            0x9A69, 0x9A6A, 
            0x9A6B, 0x9A6C, 
            0x9A6D, 0x9A6E, 
            0x9A6F, 0x9A70, 
            0x9A71, 0x9A72, 
            0x9A73, 0x9A74,// Blue Marble Fireplace
            0x9A75, 0x9A76, 
            0x9A77, 0x9A78, 
            0x9A79, 0x9A7A, 
            0x9A7B, 0x9A7C, 
            0x9A7D, 0x9A7E, 
            0x9A7F, 0x9A80// Green Marble Fireplace
		};
		...
		private static readonly Type[] m_MarkableTable = new[]
		{
			...
			typeof(BaseHarvestTool), typeof(BaseQuiver), typeof(DragonBardingDeed), typeof(Fukiya), typeof(FukiyaDart),
		...
		private void OnResourceConsumed(Item item, int amount)
		{
			...
			{
                m_PlantHue = ((PlantPigment)item).PlantHue;
				m_ResHue = item.Hue;
			}
        ...
        public void Craft(Mobile from, CraftSystem craftSystem, Type typeRes, IUsesRemaining tool)
		{
			if (from.BeginAction(typeof(CraftSystem)))
			{
				if (RequiredExpansion == Expansion.None || (from.NetState != null && from.NetState.SupportsExpansion(RequiredExpansion)))
				{
                    ...
                    if (typeof(PetAssembly).IsAssignableFrom(ItemType))
                    {
                        if (from is PlayerMobile && !((PlayerMobile)from).MechanicalLife)
                        {
                    ...
                    if (typeof(WeavableBasket).IsAssignableFrom(ItemType))
                    {
                        WeavableBasket.Craft(from, craftSystem, typeRes, tool, this);
                        return;
                    }

                    if (ItemType == typeof(NaturalDye))
                    {
                        NaturalDye.Craft(from, craftSystem, typeRes, tool, this);
                        return;
                    }

                    if (ItemType == typeof(PlantPigment))
                    {
                        PlantPigment.Craft(from, craftSystem, typeRes, tool, this);
                        return;
                    }

                    if (ItemType == typeof(SoftenedReeds))
                    {
                        SoftenedReeds.Craft(from, craftSystem, typeRes, tool, this);
                        return;
                    }
							...
							if (badCraft <= 0)
							{
								...
								if (ConsumeRes(from, typeRes, craftSystem, ref resHue, ref maxAmount, ConsumeType.None, ref message))
								{
									...
									if (ConsumeAttributes(from, ref message, false))
									{
										...
										iRandom += iMin + 1;
                                        new CraftItemTimer(from, craftSystem, this, typeRes, tool, iRandom).Start();
									}
									else
									{
										from.EndAction(typeof(CraftSystem));
        ...
        public void CompleteCraft(int quality, bool makersMark, Mobile from, CraftSystem craftSystem, Type typeRes, IUsesRemaining tool, CustomCraft customCraft)
		{
			int badCraft = craftSystem.CanCraft(from, tool, m_Type);
			...
			if (CheckSkills(from, typeRes, craftSystem, ref ignored, ref allRequiredSkills))
			{
				...
				{
                    if (tool != null && !((Item)tool).Deleted/*tool.Deleted*/ && tool.UsesRemaining > 0)
					{
						from.SendGump(new CraftGump(from, craftSystem, tool, message));
					}
					else if (message is int && (int)message > 0)
                ...
                if (CharConfig.CharHSCartographyEnabled)
                {
                    ...
                    if (item != null)
                    {
                        ...
                        bool isArtifact = CraftableArtifacts.IsCraftableArtifact(item);
                        ...
                        else if (item is BaseFood)
                        {
                            ((BaseFood)item).PlayerConstructed = true;
                        }
                        else if (item.Hue == 0)
                        {
                            item.Hue = resHue;
                        ...
                        if (context.QuestOption == CraftQuestOption.NonQuestItem)
                        {
                ...
                else if (tool != null && !((Item)tool).Deleted && tool.UsesRemaining > 0)
				{
		...
		private class CraftItemTimer : Timer
		{
            ...
            //public CraftItemTimer(Mobile from, CraftSystem craftSystem, CraftItem craftItem, Type typeRes, BaseTool tool, int iCountMax)
            public CraftItemTimer(Mobile from, CraftSystem craftSystem, CraftItem craftItem, Type typeRes, IUsesRemaining tool, int iCountMax)
			...
			protected override void OnTick()
			{
				...
				else
				{
					...
					if (badCraft > 0)
					{
                        if (m_Tool != null && !((Item)m_Tool).Deleted && m_Tool.UsesRemaining > 0)
						{
