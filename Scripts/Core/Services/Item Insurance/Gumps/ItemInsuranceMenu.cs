...
namespace Server.Gumps
{
    ...
    public class ItemInsuranceHelper
    {
        ...
        public static int GetInsuranceCost(Item item)
        {
            ...
            if ((item is BaseArmor && ((BaseArmor)item).NegativeAttributes.Prized > 0) || (item is BaseWeapon && ((BaseWeapon)item).NegativeAttributes.Prized > 0) || (item is BaseJewel && ((BaseJewel)item).NegativeAttributes.Prized > 0) || (item is BaseClothing && ((BaseClothing)item).NegativeAttributes.Prized > 0))
            {
                cost *= 2;
            }
