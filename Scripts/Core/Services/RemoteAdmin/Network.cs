...
namespace Server.RemoteAdmin
{
    public class AdminNetwork
    {
        ...
        public static void OnReceive(NetState state, PacketReader pvSrc)
        {
            ...
            {
                ConsoleLog.WriteLine("ADMIN: Unauthorized packet from {0}, disconnecting", state);
                Disconnect(state);
            }
            else
            {
        ...
        public static void Authenticate(NetState state, PacketReader pvSrc)
        {
            ...
            if (a == null)
            {
                ...
                ConsoleLog.WriteLine("ADMIN: Invalid username '{0}' from {1}", user, state);
                DelayedDisconnect(state);
            }
            else if (!a.HasAccess(state))
            {
                state.Send(new Login(LoginResponse.BadIP));
                ConsoleLog.WriteLine("ADMIN: Access to '{0}' from {1} denied.", user, state);
                DelayedDisconnect(state);
            }
            else if (!a.CheckPassword(pw))
            {
                state.Send(new Login(LoginResponse.BadPass));
                ConsoleLog.WriteLine("ADMIN: Invalid password for user '{0}' from {1}", user, state);
                DelayedDisconnect(state);
            }
            else if (a.AccessLevel < AccessLevel.Administrator || a.Banned)
            {
                ConsoleLog.WriteLine("ADMIN: Account '{0}' does not have admin access. Connection Denied.", user);
                state.Send(new Login(LoginResponse.NoAccess)); 
                DelayedDisconnect(state);
            }
            else
            {
                ConsoleLog.WriteLine("ADMIN: Access granted to '{0}' from {1}", user, state);
                state.Account = a;
                a.LogAccess(state);
        ...
        public static Packet Compress(Packet p)
        {
            ...
            if (length > 100 && length < 60000)
            {
                ...
                {
                    ConsoleLog.WriteLine("WARNING: Unable to compress admin packet, zlib error: {0}", error);
                    return p;
                }
                else
                {
