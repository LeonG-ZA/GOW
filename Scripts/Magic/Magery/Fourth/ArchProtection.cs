...
namespace Server.Spells.Fourth
{
    ...
    {
        private static readonly SpellInfo m_Info = new SpellInfo("Arch Protection", "Vas Uus Sanct", Core.AOS ? 239 : 215, 9011, Reagent.Garlic, Reagent.Ginseng, Reagent.MandrakeRoot, Reagent.SulfurousAsh);
        ...
        {
            this.Caster.Target = new ArchProtectionSpellTarget(this);
        }
                    ...
                    if (targets.Count > 0)
                    {
                        for (int i = 0; i < targets.Count; ++i)
                        {
                            ...
                            if (m.BeginAction(typeof(ArchProtectionSpell)))
                            {
                                ...
                                AddEntry(m, val);
                                new ArchProtectionSpellTimer(m, this.Caster).Start();
        ...
        private class ArchProtectionSpellTimer : Timer
        {
            private readonly Mobile m_Owner;

            public ArchProtectionSpellTimer(Mobile target, Mobile caster) : base(TimeSpan.FromSeconds(0))
            {
        ...
        private class ArchProtectionSpellTarget : Target
        {
            private readonly ArchProtectionSpell m_Owner;
            public ArchProtectionSpellTarget(ArchProtectionSpell owner) : base(Core.ML ? 10 : 12, true, TargetFlags.None)
            {
                this.m_Owner = owner;
            }
