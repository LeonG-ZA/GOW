...
namespace Server.Spells.Seventh
{
    ...
    {
        private static readonly SpellInfo m_Info = new SpellInfo("Gate Travel", "Vas Rel Por", 263, 9032, Reagent.BlackPearl, Reagent.MandrakeRoot, Reagent.SulfurousAsh);
        ...
        public override void OnCast()
        {
            if (this.m_Entry == null)
                this.Caster.Target = new GateTravelSpellTarget(this);
            else
        ...
        public override bool CheckCast()
        {
            ...
            else if (((PlayerMobile)this.Caster).Transport != null)
            {

                if (((PlayerMobile)this.Caster).Transport.IsDriven)
                    this.Caster.SendLocalizedMessage(1149592); //You are no longer piloting this vessel 

                ((PlayerMobile)this.Caster).Transport.LeaveCommand((PlayerMobile)this.Caster);
            }
        ...
        public void Effect(Point3D loc, Map map, bool checkMulti)
        {
            ...
            else if (this.CheckSequence())
            {
                ...
                if (((PlayerMobile)this.Caster).Transport != null)
                {
                    ((PlayerMobile)this.Caster).Transport.Disembark((PlayerMobile)this.Caster);
                }

                Effects.PlaySound(this.Caster.Location, this.Caster.Map, 0x20E);

                GateTravelSpellItem firstGate = new GateTravelSpellItem(loc, map);
                firstGate.MoveToWorld(this.Caster.Location, this.Caster.Map);

                Effects.PlaySound(loc, map, 0x20E);

                GateTravelSpellItem secondGate = new GateTravelSpellItem(this.Caster.Location, this.Caster.Map);
        ...
        [DispellableField]
        private class GateTravelSpellItem : Moongate
        {
            public GateTravelSpellItem(Point3D target, Map map)
                : base(target, map)
            {
                this.Map = map;
                ...
                GateTravelSpellTimer t = new GateTravelSpellTimer(this);
                t.Start();
            }

            public GateTravelSpellItem(Serial serial)
                : base(serial)
            {
            }
            ...
            private class GateTravelSpellTimer : Timer
            {
                private readonly Item m_Item;
                public GateTravelSpellTimer(Item item) : base(TimeSpan.FromSeconds(30.0))
                {
        ...
        private class GateTravelSpellTarget : Target
        {
