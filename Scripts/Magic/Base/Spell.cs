...
namespace Server.Spells
{
	public abstract class Spell : ISpell
	{
        ...
        private static readonly TimeSpan NextSpellDelay = SpellsConfig.SpellsNextSpellDelay;
        private static TimeSpan AnimateDelay = SpellsConfig.SpellsAnimateDelay;
		...
		public virtual int GetNewAosDamage(int bonus, int dice, int sides, bool playerVsPlayer, double scalar)
		{
			...
			damage = ItemAttributes.Scale(damage, evalScale);
			damage = ItemAttributes.Scale(damage, (int)(scalar * 100));
		...
		public virtual void OnCasterHurt()
		{
            ...
            if (!Caster.Player || Caster.AccessLevel >= AccessLevel.Counselor)
			{
				return;
			}
		...
		{
            if (SpellsConfig.RequiredReagents)
            {
		...
		public bool Cast()
		{
			...
			else if (m_Caster.Mana >= ScaleMana(GetMana()))
			{
				...
				if (m_Caster.Spell == null && m_Caster.CheckSpellCast(this) && CheckCast() && m_Caster.Region.OnBeginSpellCast(m_Caster, this))
				{
					...
					if (ShowHandMovement && (m_Caster.Body.IsHuman || (m_Caster.Player && m_Caster.Body.IsMonster)) && !(m_Scroll is SpellStone))
					{
		...
		public virtual bool CheckSequence()
		{
			...
			{
                if (SpellsConfig.RequiredMana)
                {
                    m_Caster.Mana -= mana;
                }

				if (m_Scroll is BaseSpellScroll)
				{
					m_Scroll.Consume();
				}
				...
				if (TransformationSpell.UnderTransformation(m_Caster, typeof(VampiricEmbraceSpell)))
				{
					...
					if (garlic)
					{
						...
						ItemAttributes.Damage(m_Caster, Utility.RandomMinMax(17, 23), 100, 0, 0, 0, 0);
					}
				}
