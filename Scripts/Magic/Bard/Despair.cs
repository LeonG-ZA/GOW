...
namespace Server.Spells.Bard
{
	public class Despair : TargetedSpellsong
	{
		...
		protected override void OnTarget( Mobile target )
		{
			if ( CheckHSequence( target ) )
			{
				...
				else
				{
					...
					{
						//new StatMod( StatType.Str, modName + "Despair Str", -strMalus )
                        new StatMod(StatType.Str, "Despair Str", -strMalus, TimeSpan.Zero)
					};
		...
		protected override void OnUpkeep()
		{
			...
			ItemAttributes.Damage( Target, Caster, m_Damage, phys, fire, cold, pois, nrgy );
