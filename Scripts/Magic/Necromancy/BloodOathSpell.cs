...
namespace Server.Spells.Necromancy
{
    public class BloodOathSpell : NecromancerSpell
    {
        ...
        private static void EventSink_BeforeDamage(BeforeDamageEventArgs e)
        {
            ...
            Mobile oath = Spells.Necromancy.BloodOathSpell.GetBloodOath(e.From);
            ...
            if (oath == e.Mobile)
            {
                 ...
                 */
                e.From.Damage((int)(e.Amount * (1 - (((e.From.Skills.MagicResist.Value * .5) + 10) / 100))), e.Mobile);
            }
        }
        ...
        private static Hashtable m_TimerTable = new Hashtable();
        ...
        public static void EndEffect(Mobile target)
        {
            if (m_OathTable.Contains(target))
            {
                Mobile caster = (Mobile)m_OathTable[target];
                ...
                m_OathTable.Remove(caster);
                m_OathTable.Remove(target);

                if (m_TimerTable.Contains(caster))
                {
                    Timer t = (ExpireTimer)m_TimerTable[caster];

                    if (t != null)
                        t.Stop();

                    m_TimerTable.Remove(caster);
                    m_TimerTable.Remove(target);
                }
        ...
        private class ExpireTimer : Timer
        {
            ...
            protected override void OnTick()
            {
                if (m_Caster.Deleted || m_Target.Deleted || !m_Caster.Alive || !m_Target.Alive || DateTime.UtcNow >= m_End)
                {
                    ...
                    m_TimerTable.Remove(m_Caster);
                    m_TimerTable.Remove(m_Target);
