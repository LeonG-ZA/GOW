	...
	public class WordOfDeathSpell : SpellweavingSpell
	{
		...
		public void Target( Mobile m )
		{
			...
			else if ( CheckHSequence( m ) )
			{
				...
				// Spellweaving Bonus
				damage = ItemAttributes.Scale( damage, (int) Caster.Skills[SkillName.Spellweaving].Value );
				...
				m.PlaySound( 0x211 );
				Effects.SendLocationParticles( new Entity( Serial.Zero, new Point3D( m.X, m.Y, m.Z + 10 ), m.Map ), 0x3779, 1, 30, 0x3, 0x3F, 0x26EC, 0x100 );
				Effects.SendMovingParticles( new Entity( Serial.Zero, new Point3D( m.X, m.Y, m.Z + 50 ), m.Map ), m, 0xF5F, 1, 0, false, false, 0x21, 0x3F, 0x251D, 0x1, 0x0, EffectLayer.Head, 0x100 );
			}
