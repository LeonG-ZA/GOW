...
namespace Server.Engines.ChampionSpawns
{
    ...
    public class PrimevalLichPuzzle : Item
    {
            ...
            : base(0x1BC3)
        {
            ...
            {
                m.SendMessage(33, "Primeval Lich champion spawn not found. Please Spawn Champions First");
                this.Delete();
            }
            else
            {
        ...
        private ChampionSpawn FindAltar()
        {
            foreach (Item item in Map.Felucca.GetItemsInRange(altarLoc, 10))
            {
                if (item is ChampionSpawn)
                {
                    ...
                    if (ChampionSpawnType.Unliving == champ.Type)
                        return champ;
                }
            }
            return null;
        ...
        private void UpdatePuzzleState(ChampionSpawn altar)
        {
            ...
            {
                if (ChampionSpawnType.Unliving != this.m_Altar.Type || !this.Active)
                    this.RemovePuzzleLevers();
