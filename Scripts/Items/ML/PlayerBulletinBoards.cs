...
namespace Server.Items
{
    ...
    public abstract class BasePlayerBB : Item, ISecurable
    {
        ...
        public class PostPrompt : Prompt
        {
            ...
            public PostPrompt(int page, BaseHouse house, BasePlayerBB board, bool greeting, int cliloc)
            {
                this.m_Page = page;
                this.m_House = house;
                this.m_Board = board;
    ...
    public class PlayerBBGump : Gump
    {
        ...
        public override void OnResponse(NetState sender, RelayInfo info)
        {
            ...
            switch ( info.ButtonID )
            {
                case 1: // Post message
                    {
                        from.Prompt = new BasePlayerBB.PostPrompt(page, house, board, false, 1062397);
                ...
                case 3: // Post greeting
                    {
                        if (house.IsOwner(from))
                        {
                            from.Prompt = new BasePlayerBB.PostPrompt(page, house, board, true, 1062404);
