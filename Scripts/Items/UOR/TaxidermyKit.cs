...
namespace Server.Items
{
    ...
    public class TaxidermyKit : Item
    {
        private static readonly TrophyInfo[] m_Table = new TrophyInfo[]
        {
            ...
            new TrophyInfo(typeof(AutumnDragonfish), 0x44E7, 1116124, 1096511),
            new TrophyInfo(typeof(BullFish), 0x4B57, 1116129, 1116190)
        };
        ...
        private class CorpseTarget : Target
        {
            ...
            protected override void OnTarget(Mobile from, object targeted)
            {
                ...
                if (!(targeted is Corpse) && !(targeted is BigFish) && !(targeted is AutumnDragonfish)&& !(targeted is BullFish))
                {
                        ...
                        for (int i = 0; i < m_Table.Length; i++)
                        {
                            if (m_Table[i].CreatureType == obj.GetType())
                            {
                                ...
                                if (pack != null && pack.ConsumeTotal(typeof(Board), 10))
                                {
                                    ...
                                    if (targeted is AutumnDragonfish)
                                    {
                                        AutumnDragonfish fish = targeted as AutumnDragonfish;
                                        ...
                                        from.AddToBackpack(new TrophyDeedHS(m_Table[i], hunter, weight, hue));

                                        this.m_Kit.Delete();
                                        return;
                                    }
                                    else if (targeted is BullFish)
                                    {
                                        BullFish fish = targeted as BullFish;
                                        ...
                                        from.AddToBackpack(new TrophyDeedHS2(m_Table[i], hunter, weight, hue));
    ...
    [Flipable(0x14F0, 0x14EF)]
    public class TrophyDeedHS2 : Item
    {
        private int m_WestID;
        private int m_NorthID;
        private int m_DeedNumber;
        ...
        private int m_FishHue;
        [Constructable]
        public TrophyDeedHS2(int westID, int northID, int deedNumber, int addonNumber)
            : this(westID, northID, deedNumber, addonNumber, null, 0, 0)
        {
        }

        public TrophyDeedHS2(int westID, int northID, int deedNumber, int addonNumber, Mobile hunter, int animalWeight, int fishHue)
            : base(0x14F0)
        {
            this.m_WestID = westID;
            this.m_NorthID = northID;
        ...
        public TrophyDeedHS2(TaxidermyKit.TrophyInfo info, Mobile hunter, int animalWeight, int fishHue)
            : this(info.NorthID - 1, info.NorthID, info.DeedNumber, info.AddonNumber, hunter, animalWeight, fishHue)
        {
        }

        public TrophyDeedHS2(Serial serial)
            : base(serial)
        {
        }
