...
namespace Server.Items
{
    public class HousePlacementTool : Item
    {
        ...
        public override void OnDoubleClick(Mobile from)
        {
            if (this.IsChildOf(from.Backpack))
            {
                ...
                if (pm != null && pm.Map == Map.TerMur && pm.LoyaltyInfo.GetValue(LoyaltyGroup.GargoyleQueen) < 10000)
                {
                    // You must rise to the rank of noble in the eyes of the Gargoyle Queen before her majesty will allow you to build a house in her lands.
                    pm.SendLocalizedMessage(1113713);
                    return;
                }
    ...
    {
        public override int TypeID { get { return 0x1D0; } }
    ...
    public class NewHousePlacementTarget : MultiTarget
    {
        ...
        protected override void OnTarget(Mobile from, object o)
        {
            ...
            if (ip != null)
            {
                ...
                if (from.Map == Map.TerMur && from is PlayerMobile && ((PlayerMobile)from).LoyaltyInfo.GetValue(LoyaltyGroup.GargoyleQueen) < 10000)
                {
                    // You must rise to the rank of noble in the eyes of the Gargoyle Queen before her majesty will allow you to build a house in her lands.
                    from.SendLocalizedMessage(1113713);
                }
                else if (acc == null || acc.Trial)
                {
                    from.SendLocalizedMessage(1111864); // Trial account players may not place, trade, receive, or own a house.
                }
                else if (from.AccessLevel >= AccessLevel.GameMaster || reg.AllowHousing(from, p))
