...
namespace Server.Gumps
{
    ...
    public class HouseGumpAOS : Gump
    {
        ...
        private static readonly int[] m_FoundationNumbersTOL = (Core.TOL ? new int[]
        {
            20, 189, 765, 65, 101, 0x2DF7, 0x2DFB, 0x3672, 0x3676, 0x9ABE, 0x9BD0, 0x41A6, 0x4157,
        } : new int[]
            {
                20, 189, 765, 65, 101
            });
            ...
            : base(50, 40)
        {
            ...
            else
            {
                ...
                {
                    AddButtonLabeled(140, 390, GetButtonID(0, 0), 1060674); // Banish		10, 390,
                    AddButtonLabeled(140, 410, GetButtonID(0, 1), 1011261); // Lift a Ban	10, 410,
                }
            }
                ...
                case HouseGumpPageAOS.Security:
                    {
                        ...
                        else
                        {
                            if (T2AConfig.T2AHouseDoorKeysEnabled)
                            {
                                ...
                                AddButtonLabeled(200, 310, GetButtonID(3, 8), 1011260); // View Ban List		10, 310,
                                AddButtonLabeled(200, 330, GetButtonID(3, 9), 1060698); // Clear Ban List		10, 330,
                                ...
                                AddButtonLabeled(210, 170, GetButtonID(3, 14), 1011247, isOwner); // Change the house locks
                            }
                            else
                            {
                ...
                case HouseGumpPageAOS.ChangeFoundation:
                    {
                        if (Core.TOL)
                        {
                            ...
                            for (int i = 0; i < 2; ++i)
                            {
                                ...
                                for (int j = 0; j < 10 && index < m_FoundationNumbersTOL.Length; ++j)
                                {
                                    int x = 15 + ((j % 5) * 80);
                                    int y = 150 + ((j / 5) * 100);

                                    AddButton(x, y, 4005, 4007, GetButtonID(8, index), GumpButtonType.Reply, 0);
                                    AddItem(x + 25, y, m_FoundationNumbersTOL[index++]);
                                }
                            }
                            ...
                            case 0: // Banish
                                {
                                    if (T2AConfig.T2AHouseDoorKeysEnabled)
                                    {
                                        if (m_House.Public || !m_House.Public)
                                        {
                            ...
                            case 0: // Convert Into Customizable House
                                {
                                    if (isOwner && !isCustomizable)
                                    {
                                        ...
                                        {
                                            if (from.HasGump(typeof(WarningGump)))
                            ...
                            case 7: // Rename House
                                {
                                    if (isCoOwner)
                                    {
                                        //from.SendGump(new RenameHouseGump(this, from));
