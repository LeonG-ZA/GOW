...
namespace Server.Multis
{
    ...
    public abstract class BaseBoat : BaseMulti
    {
        ...
        public int NextNavPoint
        {
            get
            {
                return m_NextNavPoint;
            }
            set
            {
                m_NextNavPoint = value;
            }
        }
        [CommandProperty(AccessLevel.GameMaster)]
        ...
        public void BeginDryDock(Mobile from)
        {
            ...
            if (result == DryDockResult.Dead)
                ...
                from.CloseGump(typeof(ConfirmDryDockGump));
                from.SendGump(new ConfirmDryDockGump(from, this));
        }
        public void BeginClassicDryDock(Mobile from, Mobile master)
        {
            if (CheckDecay())
                return;
            /*FIND GOLD*/
            ...
            {
                EndClassicDryDock(from, master, bank);
            }
        ...
        public void EndClassicDryDock(Mobile from, Mobile master, Container bank)
        {
            if (Deleted || CheckDecay())
                return;
            ...
            from.AddToBackpack(boat);
            Delete();
            bank.ConsumeTotal(typeof(Gold), 25);//Set Price
