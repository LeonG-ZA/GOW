...
namespace Server.Multis
{
    ...
    public abstract class BaseShip : BaseSmoothMulti
    {
        ...
        [CommandProperty(AccessLevel.GameMaster)]
        public ushort Durability { get { return m_Durability; } set { SetDurability(value); if (m_TillerManMobile != null) m_TillerManMobile.InvalidateProperties(); } }
        ...
        public override void OnSpeech(SpeechEventArgs e)
        {
            ...
            if (IsOnBoard(from))
            {
				...
				if (this.Owner != null && this.Owner != from)
				{
                    ...
                    else if (this.Guild != 0 && from.Guild == this.Owner.Guild && this.Guild < 2)
					{
						return;
					}
                    else if (this.Party != 0 && from.Party == this.Owner.Party && this.Party < 2)
					{
						return;
					}
                    else if (this.Public != 0 && this.Public < 2)
					{
						return;
					}
				}
		...
		public virtual uint CreateKeys( Mobile m )
		{
			...
			packKey.Name = "ship recall rune";
            bankKey.Name = "ship recall rune";

            packKey.Description = ShipName;
            bankKey.Description = ShipName;
			...
			if ( !box.TryDropItem( m, bankKey, false ) )
                ...
                m.SendLocalizedMessage(1149581); // A recall rune for your new ship has been placed in your bank box.

			if ( m.AddToBackpack( packKey ) )
                m.SendLocalizedMessage(1149580); // A recall rune to your ship has been placed in your backpack.
			else
                m.SendLocalizedMessage(501552); //Thy pack seems full, so I will put it at your feet.
