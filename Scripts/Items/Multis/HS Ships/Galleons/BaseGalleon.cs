...
namespace Server.Multis
{
    ...
    public abstract class BaseGalleon : BaseShip
    {
        ...
        protected BaseGalleon(int northMultiID) : base(northMultiID, 100)
        {
        ...
        public BaseGalleon(Serial serial) : base(serial)
        { 
        }
        ...
        protected override bool EndMove()
        {
            ...
            Speed = SpeedCode.Stop;

            //Invalidate Mobile Tillerman's properties so it can be accessed immediately after the Galleons stops (Only works on Galleons)
            if (TillerManMobile != null)
                TillerManMobile.InvalidateProperties();
        ...
        public void BeginDryDock(Mobile from)
        {
            ...
            if (result == DryDockResult.Dead)
                ...
                from.CloseGump(typeof(GalleonConfirmDryDockGump));
		...
		{
			if (m_RepairTime < DateTime.UtcNow)
			{
				//TillerManMobile.Say(1116597, "5, 5, 5");
				Durability = 50;
				m_RepairTime = DateTime.UtcNow + TimeSpan.FromMinutes(5.0);
			}
			else
			{
				TimeSpan TimeBeforeRepair = m_RepairTime - DateTime.UtcNow;
