...
namespace Server.Items
{
    ...
    public class SingleHelm : BaseGalleonItem, IHelm
    {
       private BaseGalleon m_Galleon;
        ...
        public SingleHelm(BaseGalleon galleon, int northItemID, Point3D initOffset)  : base(galleon, northItemID, initOffset)
        {
            m_Galleon = galleon;
            Name = "Wheel";
        }

        public SingleHelm(Serial serial) : base(serial)
        {
        }

        public override void OnDoubleClick(Mobile from)
        {
            if (!Transport.IsDriven)//&& (Utility.InRange(this.Location, from.Location, 2))
            {
                if (!Transport.Contains(from))
                {
                ...
                else if (from == m_Galleon.Owner)
                {
                    from.SendLocalizedMessage(1116727);//You are now piloting this vessel.
                ...
                else if (m_Galleon.PlayerAccess != null)
                {
                    ...
                    {
                        if (m_Galleon.PlayerAccess[(PlayerMobile)from] >= 2 && m_Galleon.PlayerAccess[(PlayerMobile)from] <= 4)
                        {
                            from.SendLocalizedMessage(1116727);//You are now piloting this vessel.
                            Transport.TakeCommand(from);
                        }
                        else if (((from.Guild == m_Galleon.Owner.Guild) && (from.Guild != null)) || ((from.Party == m_Galleon.Owner.Party) && (from.Party != null)))
                        {
                            if ((m_Galleon.Guild >= 2 && m_Galleon.Guild <= 4) || (m_Galleon.Party >= 2 && m_Galleon.Party <= 4) || (m_Galleon.Public >= 2 && m_Galleon.Public <= 4))
                            {
                                from.SendLocalizedMessage(1116727);//You are now piloting this vessel.
                                Transport.TakeCommand(from);
                            }
    ...
    public class MultiHelm : BaseGalleonMultiItem, IHelm
    {
        ...
        public MultiHelm(BaseGalleon galleon, int northItemID, Point3D initOffset, int sxNorthItemId, int dxNorthItemId) : base(galleon, northItemID, initOffset)
        {
            m_Galleon = galleon;
        ...
        public MultiHelm(BaseGalleon galleon, int northItemID, Point3D initOffset, int relatedNorthItemId, Point3D relatedOffset) : base(galleon, northItemID, initOffset)
        {
            m_Galleon = galleon;
    ...
    public class GalleonHold : BaseGalleonMultiContainer
    {
        ...
        public override void OnDoubleClick(Mobile from)
        {
            if (m_Galleon == null || !m_Galleon.IsOnBoard(from))
            {
                if (m_Galleon.TillerManItem != null)
                    m_Galleon.TillerManItem.Say(502490); // You must be on the ship to open the hold.
                if (m_Galleon.TillerManMobile != null)
                    m_Galleon.TillerManMobile.TillerManSay(502490); // You must be on the ship to open the hold.
            }
            else if (m_Galleon.IsMovingShip || Transport.IsDriven)
            {
                if (m_Galleon.TillerManItem != null)
                    m_Galleon.TillerManItem.Say(502491); // I can not open the hold while the ship is moving.
                if (m_Galleon.TillerManMobile != null)
                    m_Galleon.TillerManMobile.TillerManSay(502491); // I can not open the hold while the ship is moving.
            }
            else
            {
                if (m_Galleon != null)
                {
                    ...
                    {
                        if ((m_Galleon.PlayerAccess.ContainsKey((PlayerMobile)from)) || ((from.Guild == m_Galleon.Owner.Guild) && (from.Guild != null)) || ((from.Party == m_Galleon.Owner.Party) && (from.Party != null)))
                        {
                            if ((m_Galleon.PlayerAccess[(PlayerMobile)from] == 3) || (m_Galleon.PlayerAccess[(PlayerMobile)from] == 4) || (m_Galleon.Guild == 3) || (m_Galleon.Guild == 4) || (m_Galleon.Party == 3) || (m_Galleon.Party == 4) || (m_Galleon.Public == 3) || (m_Galleon.Public == 4))
                            {
                                base.OnDoubleClick(from);
                            }
                            else
    ...
    {
        private BaseGalleon Galleon { get { return Transport as BaseGalleon; } }
        ...
        public MainMast(BaseGalleon galleon, int baseNorthId, Point3D initOffset) : base(galleon, baseNorthId, initOffset)
        {
        }

        public MainMast(Serial serial) : base(serial)
        {
        }
        ...
        public override void OnDoubleClick(Mobile from)
        {
            if (m_Boat != null && !m_Boat.Contains(from))
            {
                ...
                else if (m_Boat.PlayerAccess != null)
                {
                    ...
                    {
                        if (((m_Boat.Owner != null) && (from.Guild == m_Boat.Owner.Guild) && (from.Guild != null)) || ((m_Boat.Owner != null) && (from.Party == m_Boat.Owner.Party) && (from.Party != null)))
                        {
                            if ((m_Boat.PlayerAccess[(PlayerMobile)from] >= 1 && m_Boat.PlayerAccess[(PlayerMobile)from] <= 4) || (m_Boat.Guild >= 1 && m_Boat.Guild <= 4) || (m_Boat.Party >= 1 && m_Boat.Party <= 4) || (m_Boat.Public >= 1 && m_Boat.Public <= 4))
                            {
                                base.OnDoubleClick(from);
                                from.MoveToWorld(this.Location, this.Map);
                                Transport.Embark(from);
