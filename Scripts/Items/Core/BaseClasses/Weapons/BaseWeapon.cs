...
namespace Server.Items
{
    ...
    public abstract class BaseWeapon : Item, IWeapon, IFactionItem, ICraftable, ISlayer, IDurability, ISetItem, IArtifactRarity, IGorgonCharges, IImbuable
	{
        ...
        #region Weapons Internals Comments
        ...
        */
		...
		public int StrRequirement
		{
			get 
            {
                ...
                return Math.Min(110, (int)((double)(m_StrReq == -1 ? Core.AOS ? AosStrengthReq : OldStrengthReq : m_StrReq) * (m_NegativeAttributes.Massive > 0 ? 1.5 : 1)));
            }
			set
			{
				m_StrReq = value;
		...
		public override bool CanEquip(Mobile from)
		{
            ...
            if (MainConfig.MainItemIDEnabled || T2AConfig.T2AItemIDEnabled)
            {
                ...
                else if (from.Str < ItemAttributes.Scale(StrRequirement, 100 - GetLowerStatReq()))
                {
		...
		public virtual bool CheckHit(Mobile attacker, Mobile defender)
		{
			...
			if (Core.AOS)
			{
                ...
                //Gargoyle Cap of 50
                bonus = Math.Min(attacker.Race == Race.Gargoyle ? 45 : 45, bonus);
            ...
            if (atkWeapon is BaseThrown && ((BaseThrown)atkWeapon).Debug)
            {
                attacker.SendMessage("Your weapon's range is {0} of {1}", atkWeapon.GetMaxRange(attacker, defender), atkWeapon.DefMaxRange);
            ...
            BaseThrown.ApplyShieldPenalties(attacker, defender, ref chance);
        ...
        public virtual double GetAccuracyScalar(Mobile attacker, Mobile defender, int defmaxRange)
        {
            return 1.0;
        }

        public virtual double GetDamageScalar(Mobile attacker, Mobile defender, int defmaxRange)
        {
            return 1.0;
        }
		...
		public virtual TimeSpan GetDelay(Mobile m)
		{
			...
			else if (Core.AOS)
			{
                ...
                v += ItemAttributes.Scale(v, bonus);
		...
		public virtual TimeSpan OnSwing(Mobile attacker, Mobile defender, double damageBonus)
		{
            ...
            if (canSwing)
            {
                canSwing = !BaseBardCreature.IsCalmed(attacker);
            }
        ...
        public virtual int AbsorbDamageTOL(Mobile attacker, Mobile defender, int damage)
        {
            ...
            {
                ItemAttributes.Damage(attacker, defender, Utility.Random(15, 25), 0, 0, 0, 0, 100); // TODO: verify
        ...
        {  
            Layer.Talisman,   Layer.InnerLegs,  Layer.InnerTorso,  Layer.MiddleTorso,
            Layer.Waist,      Layer.Cloak,      Layer.OuterTorso,  Layer.Ring,
            Layer.Bracelet,   Layer.Neck,       Layer.Neck,        Layer.Gloves,
            Layer.Gloves,     Layer.Arms,       Layer.Arms,        Layer.Helm,
            Layer.Helm,       Layer.Pants,      Layer.Pants,       Layer.Pants,
            Layer.Shirt,      Layer.Shirt,      Layer.Shirt
        };
		...
		public virtual int AbsorbDamage(Mobile attacker, Mobile defender, int damage)
		{
            if (Core.TOL)
            {
                return AbsorbDamageTOL(attacker, defender, damage);
            }
		...
		public virtual void OnHit(Mobile attacker, Mobile defender, double damageBonus)
		{
			...
			#region Damage Multipliers Comment
            /*
            ...
            */
            ...
            damage = ItemAttributes.Scale(damage, 100 + percentageBonus);
            ...
            if (m_Immolating)
            {
                ...
                ItemAttributes.Damage(defender, attacker, d, 0, 100, 0, 0, 0);
            }
            #endregion
            ...
            if (m_SearingWeapon && attacker.Mana > 0)
            {
                ...
                {
                    ItemAttributes.Damage(defender, attacker, d, 0, 100, 0, 0, 0);
                    ItemAttributes.Damage(attacker, null, 4, false, 0, 0, 0, 0, 0, 0, 100, false, false, false);
            ...
            if (cloak is BaseQuiver && twohanded is BaseRanged)
            {
                ...
                int dmod = quiver.DamageIncrease;
                ...
                if (dmod > 0)
                {
                    ...
                    chaos += quiver.ChaosDamage * dmod / 100;
                }
            }
            #endregion
            ...
            if (m_AosWeaponAttributes.Velocity > Utility.RandomMinMax(1, 100))
            {
                ...
                ItemAttributes.Damage(defender, attacker, damageVelocity, 100, 0, 0, 0, 0);
            ...
            // New stuff for BladeWeaving performing Armor Ignore attack
            ...
            damageGiven = ItemAttributes.Damage(defender,attacker,damage,ignoreArmor,phys,fire,cold,pois,nrgy,chaos,direct,false,this is BaseRanged,false);
			...
			if (Core.AOS)
			{
				...
				if (context != null && context.Type == typeof(WraithFormSpell))
				{
                    ...
                    defender.Mana -= ItemAttributes.Scale(damageGiven, wraithLeech);
				...
				if (lifeLeech != 0)
				{
                    //attacker.Hits += ItemAttributes.Scale(damageGiven, lifeLeech);
                    int toHeal = ItemAttributes.Scale(damageGiven, lifeLeech);
                    #region High Seas
                    if (defender is BaseCreature && ((BaseCreature)defender).TaintedLifeAura)
                    {
                        ItemAttributes.Damage(attacker, defender, toHeal, false, 0, 0, 0, 0, 0, 0, 100, false, false, false);
				...
				if (stamLeech != 0)
				{
                    attacker.Stam += ItemAttributes.Scale(damageGiven, stamLeech);
				}

				if (manaLeech != 0)
				{
                    attacker.Mana += ItemAttributes.Scale(damageGiven, manaLeech);
				}
            ...
            bool acidBlood = defender is Slime || defender is AcidElemental || defender is UnderworldSlime || defender is AcidSlug;
			...
			if (Core.AOS)
			{
                ...
                int splinteringChance = (int)(SAAbsorptionAttributes.GetValue(attacker, SAAbsorptionAttribute.SplinteringWeapon) * propertyBonus);
                ...
                if (splinteringChance != 0 && splinteringChance > Utility.Random(100))
                {
                    DoSplintering(attacker, defender);
                }
		...
		public virtual double GetAosDamage(Mobile attacker, int bonus, int dice, int sides)
		{
            ...
            damage = ItemAttributes.Scale(damage, 100 + damageBonus);
		...
		public virtual void DoCurse(Mobile attacker, Mobile defender)
		{
            ...
            CurseSpell sp = new CurseSpell(attacker, null);
        ...
        public virtual void DoSplintering(Mobile attacker, Mobile defender)
        {
            int damage = 2;
            ...
            defender.LocalOverheadMessage(MessageType.Regular, 0x21, 1060757);
            ...
            new SpeedContext(defender, SpeedContext.SpeedState.Slow, "SpeedBoost");
            new SplinteringTimer(defender, TimeSpan.FromSeconds(4.0)).Start();
        }

        private class SplinteringTimer : Timer
        {
            private readonly Mobile m_Mobile;

            public SplinteringTimer(Mobile m, TimeSpan duration)
                : base(duration)
            {
                m_Mobile = m;
            }
            ...
            protected override void OnTick()
            {
                SpeedContext.RemoveContext(m_Mobile, "SpeedBoost");
            }
        }
		...
		public virtual void DoAreaAttack(Mobile from, Mobile defender, int sound, int hue, int phys, int fire, int cold, int pois, int nrgy)
		{
			...
			for (int i = 0; i < list.Count; ++i)
			{
                ...
                ItemAttributes.Damage(m, from, (int)damage, phys, fire, cold, pois, nrgy); 
			}
		}
		#endregion
		...
		public virtual double GetBaseDamage(Mobile attacker)
		{
            ...
            #region Apply damage level comment
 ...
 */
		...
		public virtual int ScaleDamageByDurability(int damage)
		{
            ...
            return ItemAttributes.Scale(damage, scale);
		}
		...
		[Flags]
		private enum SaveFlag : long //uint
		{
			None = 0x00000000,
            ...
            Altered = 0x100000000,
            xNegativeAttributes = 0x200000000
		}
				...
				case 5:
					{
                      //SaveFlag flags = (SaveFlag)reader.ReadInt();
                        ...
                        if (version >= 15 && GetSaveFlag(flags, SaveFlag.xNegativeAttributes))  
                        {  
        ...
        public override void GetProperties(ObjectPropertyList list)
		{
            ...
            if (MainConfig.MainItemIDEnabled || T2AConfig.T2AItemIDEnabled)
            {
                if (m_Identified /*|| from.AccessLevel >= AccessLevel.GameMaster*/)
                {
                    ...
                    {
                        list.Add(1151782);// cannot repair
                    }
                   ...
                   // }
                    ...
                    if ((prop = m_SAAbsorptionAttributes.BattleLust) != 0)
                    {
                        list.Add(1113710, prop.ToString()); // Battle Lust
                    }

                    if ((prop = m_SAAbsorptionAttributes.BloodDrinker) != 0)
                    {
                        list.Add(1113591, prop.ToString()); // Blood Drinker
                    }

                    if ((prop = m_AosWeaponAttributes.Balanced) != 0)
                    {
                        list.Add(1072792); // Balanced
                    }

                    if ((prop = m_AosWeaponAttributes.Velocity) != 0)
                    {
                    ...
                    if ((prop = m_SAAbsorptionAttributes.SplinteringWeapon) != 0)
                    {
                    ...
                    int strReq = ItemAttributes.Scale(StrRequirement, 100 - GetLowerStatReq());
                    ...
                    XmlAttach.AddAttachmentProperties(this, list);
                ...
                else if ((m_AosWeaponAttributes.UseBestSkill) != 0 || ((GetDamageBonus() + m_AosAttributes.WeaponDamage)) != 0 ||
                        ...
                        (m_AosAttributes.BonusStr) != 0 || (m_AosAttributes.BonusStam) != 0 || (m_AosAttributes.WeaponSpeed) != 0 || (m_SAAbsorptionAttributes.CastingFocus) != 0)
