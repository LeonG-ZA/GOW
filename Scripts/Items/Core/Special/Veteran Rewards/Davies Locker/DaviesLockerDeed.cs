	...
	public class DaviesLockerDeed : Item
	{
		...
		public override void Serialize( GenericWriter writer )
		{
			...
			for ( int i = 0; i < m_Entries.Count; ++i )
				((DaviesDeedEntry)m_Entries[i]).Serialize( writer );
		...
		public bool ValidatePlacement( Mobile from, Point3D loc,  TableType type  )
		{
			...
			if (type == TableType.East)
				...
				else if ( !map.CanFit( x, y2, z, 20 , true, false, false ) )
					{
					from.SendLocalizedMessage( 500269 ); // You cannot build that there.
					return false;
					}
				...
				else if ( !map.CanFit( x2, y, z, 20 , true, false, false ) )
					{
					from.SendLocalizedMessage( 500269 ); // You cannot build that there.
					return false;
					}
	...
	public class DaviesDeedEntry
		{
			...
			public DaviesDeedEntry( int type, int level, Mobile decoder, Map map, Point2D loc2d, Point3D loc3d, Rectangle2D bounds, string number ) 
			{
				m_type = type; 
				m_Level = level;
				m_Decoder = decoder;
