...
namespace Server.Gumps
{
	public class DaviesLockerGump : Gump
	{
		...
		public DaviesLocker Table{ get{ return m_Table; } }
		...
		private void AddDetails( int index, int count )
		{
			...
			if ( index < m_Table.Entries.Count )
			{
				...
				else
				{
					...
					{
						if(e.Mapnumber == "Unknown Spot")
						{
						AddLabel( 417, 167 +( count * 35), hue, String.Format( "{0}", e.Location2d ) );	
						}
						else
						{
						AddLabel( 417, 167 +( count * 35), hue, String.Format( "{0}", e.Mapnumber ) );		//Coords - TMAP 
						}
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			if ( m_Table.Deleted || !from.InRange( m_Table.GetWorldLocation(), 3 ) || !Multis.DesignContext.Check( from ) )
				return;
		...
		private class DaviesLockerTarget : Target
      {
			...
			protected override void OnTarget( Mobile from, object targeted )
			{
				...
				if( targeted is Item )
				{
					if( m_Table.OnDragDrop( from, targeted as Item ) )
						from.SendMessage( "Added, please choose another." );
					else
						from.SendMessage( "Invalid item, please try another." );
