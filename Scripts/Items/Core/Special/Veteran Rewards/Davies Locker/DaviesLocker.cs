...
namespace Server.Items
{
	...
	public class DaviesLocker : Item, IAddon, IChopable
	{
		...
		private class LockerPiece : Item, IChopable
		{
			...
			public override int LabelNumber{ get{ return 1153534; } } // Davies Locker
		...
		Item IAddon.Deed
		{
			...
			for ( int i = 0; i < m_Entries.Count; ++i )
					...
					DaviesLockerEntry e = (DaviesLockerEntry) this.Entries[i];
					deed.m_Entries.Add( new DaviesDeedEntry( e.type, e.Level, e.Decoder, e.Map, e.Location2d, e.Location3d, e.Bounds, e.Mapnumber ) );  
		...
		public override void Serialize( GenericWriter writer )
		{
			...
			for ( int j = 0; j < m_Entries.Count; ++j )
				{
				((DaviesLockerEntry)m_Entries[j]).Serialize( writer );
				}
		...
		public void DropTMap( Mobile from, DaviesLockerEntry e, int index )
		{
			...
			tmap.ChestLocation = e.Location2d;
			tmap.ChestMap = e.Map;
			...
			SOS sos = new SOS(e.Map, e.Level); 
				...
				sos.TargetLocation = e.Location3d; 
		...
		public void ViewMap( Mobile from, DaviesLockerEntry e, int index )
		{
			...
			from.SendGump( new DaviesMapGump( from, e.Location2d.X, e.Location2d.Y ) );
		}
		...
		{ 
		  ...
		  public static readonly TMList MapLocations1 = 
			...
			public static readonly TMList[] UORLists = new TMList[] { MapLocations1 };
	...
	public class DaviesLockerEntry
		{
			...
			public DaviesLockerEntry( int type, int level, Mobile decoder, Map map, Point2D loc2d, Point3D loc3d, Rectangle2D bounds, string number ) 
			{
				m_type = type; 
				m_Level = level;
				m_Decoder = decoder;
