...
namespace Server.Items
{
	...
	public class Corpse : Container, ICarvable
	{
		...
		public void BeginDecay(TimeSpan delay)
		{
            ...
            m_DecayTimer = new CorpseTimer(this, delay);
			m_DecayTimer.Start();
		}
		...
		private class CorpseTimer : Timer
		{
			private readonly Corpse m_Corpse;

            public CorpseTimer(Corpse c, TimeSpan delay)
				: base(delay)
			{
				m_Corpse = c;
		...
		public bool CheckLoot(Mobile from, Item item)
		{
            ...
            else if ((map == null || (map.Rules & MapRules.HarmfulRestrictions) != 0) && m_Owner != null && m_Owner.Player && NotorietyHandlers.CorpseNotoriety(from, this) == Notoriety.Criminal)
            {
		...
		public virtual void Open(Mobile from, bool checkSelfLoot)
		{
			if (from.IsStaff() || from.InRange(GetWorldLocation(), 2))
			{
				...
				if (selfLoot)
				{
                    if (from is PlayerMobile && from.NetState != null && from.NetState.IsKRClient)
                    {
