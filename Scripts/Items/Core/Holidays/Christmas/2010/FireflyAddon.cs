...
{
	public class FireflyAddon : Item, IDyable, IAddon
	{
		[Constructable]
		public FireflyAddon() : this( Utility.RandomDyedHue() )
		{
		}

		[Constructable]
		public FireflyAddon( int hue ) : base( 0x2336 )
		{
			Hue = 0;
			Movable = false;
		}

		public FireflyAddon( Serial serial ) : base( serial )
		{
		}
		...
		public Item Deed
		{
			get{ return new FireflyDeed( this.Hue ); }
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			if ( house != null && house.IsCoOwner( from ) )
			{
				...
				{
					from.CloseGump( typeof( FireflyAddonGump ) );
					from.SendGump( new FireflyAddonGump( from, this ) );
				}
				else
				{
		...
		private class FireflyAddonGump : Gump
		{
			private Mobile m_From;
			private FireflyAddon m_Addon;

			public FireflyAddonGump( Mobile from, FireflyAddon addon ) : base( 150, 50 )
			{
				m_From = from;
				m_Addon = addon;
	...
	public class FireflyDeed : Item
	{
		public override int LabelNumber{ get{ return 1150061; } } // holiday fireflies

		[Constructable]
		public FireflyDeed() : this( Utility.RandomDyedHue() )
		{
		}

		[Constructable]
		public FireflyDeed(int hue) : base( 0x14F0 )
		{
			Weight = 1.0;
			Hue = 0;
		...
		public FireflyDeed( Serial serial ) : base( serial )
		{
		}
		...
		public void Placement_OnTarget( Mobile from, object targeted, object state )
		{
			...
			if ( house != null && house.IsCoOwner( from ) )
			{
				...
				if ( northWall && westWall )
					from.SendGump( new FireflyDeedGump( from, loc, this ) );
				else
		...
		private void PlaceAddon( Mobile from, Point3D loc, bool northWall, bool westWall )
		{
			...
			if ( itemID > 0 )
			{
				Item addon = new FireflyAddon( this.Hue );
		...
		private class FireflyDeedGump : Gump
		{
			private Mobile m_From;
			private Point3D m_Loc;
			private FireflyDeed m_Deed;

			public FireflyDeedGump( Mobile from, Point3D loc, FireflyDeed deed ) : base( 150, 50 )
			{
				m_From = from;
				m_Loc = loc;
				m_Deed = deed;
