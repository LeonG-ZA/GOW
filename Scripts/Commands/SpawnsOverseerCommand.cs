...
namespace Server.Commands
{
	public class GenOverseer
	{
		...
		private static void GenOverseer_OnCommand( CommandEventArgs e )
		{
			...
			Generate( "Data/Spawns/overseers/Trammel", Map.Trammel );
			Generate( "Data/Spawns/overseers/Felucca", Map.Felucca );
			Generate( "Data/Spawns/overseers/Ilshenar", Map.Ilshenar );
			Generate( "Data/Spawns/overseers/Malas", Map.Malas );
			Generate( "Data/Spawns/overseers/Tokuno", Map.Tokuno );
			Generate( "Data/Spawns/overseers/Termur", Map.TerMur );
	...
	public class RemOverseer
	{
		...
		public static void RemOverseers_OnCommand( CommandEventArgs e )
		{
			...
			{ 
				if ( premiums is PrimeSpawner && premiums.Parent == null && ((PrimeSpawner)premiums).Running == false )
				{
					((PrimeSpawner)premiums).Running = true;
					((PrimeSpawner)premiums).NextSpawn = TimeSpan.FromSeconds( 1 );
				}
			}
