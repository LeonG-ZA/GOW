...
{
    public class GenTeleporterT2A
    {
        public GenTeleporterT2A()
        {
        }

        public static void Initialize()
        {
            CommandSystem.Register("TelGenT2A", AccessLevel.Administrator, new CommandEventHandler(GenTeleporterT2A_OnCommand));
        }

        [Usage("TelGenT2A")]
        [Description("Generates world/dungeon teleporters for T2A.")]
        public static void GenTeleporterT2A_OnCommand(CommandEventArgs e)
        {
            e.Mobile.SendMessage("Generating teleporters, please wait.");

            int count = new TeleportersCreatorT2A().CreateTeleporters();

            e.Mobile.SendMessage("Teleporter generating complete. {0} teleporters were generated.", count);
        }

        public class TeleportersCreatorT2A
        {
            private static readonly Queue m_Queue = new Queue();
            private int m_Count;
            public TeleportersCreatorT2A()
            {
            }
