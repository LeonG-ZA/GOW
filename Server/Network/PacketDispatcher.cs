...
namespace Server.Network
{
	public static class PacketDispatcher
	{
			...
			if ( map != null )
			{
				...
				foreach ( NetState state in map.GetClientsInRange( m.Location ) )
				{
					if ( state.Mobile.CanSee( m ) )
					{
						...
						if ( p == null )
                            p = Packet.Acquire(new NewMobileAnimation(m, action, subAction));
		...
		private static void Mobile_ItemLifted( Mobile m, ItemLiftedEventArgs args )
		{
			...
			if ( liftSound != -1 )
				m.Send(new PlaySound( liftSound, m ) );
		}
		...
		private static void Mobile_LocationChanged( Mobile from, LocationChangedEventArgs args )
		{
			...
			if ( map != null )
			{
				...
				{
                    if (ns != from.NetState && !Utility.InUpdateRange(newLocation, ns.Mobile.Location))
					{
						if ( removeThis == null )
				...
				if ( ourState != null )
				{
					...
					foreach ( object o in map.GetObjectsInRange( newLocation, Mobile.GlobalMaxUpdateRange ) )
					{
						if ( o is Item )
						{
                            ...
                            if (!Utility.InUpdateRange(item, oldLocation) && Utility.InUpdateRange(item, newLocation) && from.CanSee(item))
						...
						else if ( o != from && o is Mobile )
						{
                            ...
                            if (!Utility.InUpdateRange(m, newLocation))
								continue;

                            bool inOldRange = Utility.InUpdateRange(m, oldLocation);
				...
				else
				{
					...
					{
                        if ((isTeleport || !Utility.InUpdateRange(oldLocation, ns.Mobile.Location)) && ns.Mobile.CanSee(from))
						{
			...
			if ( from.Map != null && ns != null )
			{
				foreach ( object o in from.Map.GetObjectsInRange( from.Location, Mobile.GlobalMaxUpdateRange ) )
				{
					if ( o is Mobile )
					{
                        ...
                        if (m != from && Utility.InUpdateRange(m, from))
					...
					else if ( o is Item )
					{
                        ...
                        if (Utility.InUpdateRange(item, from))
			...
			if ( from.Map != null && ns != null )
			{
				foreach ( object o in from.Map.GetObjectsInRange( from.Location, Mobile.GlobalMaxUpdateRange ) )
				{
					...
					else if ( o is Mobile )
					{
                        ...
                        if (from.CanSee(m) && Utility.InUpdateRange(from, m))
						{
