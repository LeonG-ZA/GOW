...
namespace Server.Network
{
	...
	public class PacketHandlers
	{
		...
		static PacketHandlers()
		{
			...
			Register(0xBD, 0, true, ClientVersion);// false
            ...
            RegisterExtended(0x32, true, RacialAbility);
        ...
        public static void DropReq6017(NetState state, PacketReader pvSrc)
        {
            ...
            if (dest.IsMobile)
            {
                ...
                {
                    Console.WriteLine("Exception disarmed in drop {0} > {1}: {2}", from, m, e);
                }
            }
            else if (dest.IsItem)
            {
                ...
                {
                    Console.WriteLine("Exception disarmed in drop {0} > {1}: {2}", from, i, e);
                }
            }
            else
            {
        ...
        public static void DisplayGumpResponse(NetState state, PacketReader pvSrc)
        {
            ...
            foreach (Gump gump in state.Gumps)
            {
                if (gump.Serial == serial && gump.TypeID == typeID)
                {
                    ...
                    {
                        Console.WriteLine("Exception disarmed in gump response of {0}: {1}", gump, e);
                    }
        ...
        public static void RacialAbility(NetState state, PacketReader pvSrc)
        {
            ...
            {
                Console.WriteLine("Exception disarmed in RacialAbility {0}, ability {1}: {2}", from, abilityId, e);
            }
        }
