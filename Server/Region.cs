    ...
    public class Region : IComparable
    {
        ...
        public void Unregister()
        {
            ...
            if (this.m_Children.Count > 0)
            {
                ...
                ConsoleLog.WriteLine("G.O.W: Warning - Unregistering region '{0}' with children", new object[]
				{
					this
				});
                Utility.PopColor();
        ...
        {
            if (GOWLoggingConfig.NewRegionSystemEnabled)
            {
                if (!File.Exists("Data/Regions/Trammel.xml"))
                {
                    Utility.PushColor(ConsoleColor.White);
                    ConsoleLog.Write("G.O.W: Data/Regions/Trammel.xml does not exist");
                ...
                else if (!File.Exists("Data/Regions/Felucca.xml"))
                {
                    Utility.PushColor(ConsoleColor.White);
                    ConsoleLog.Write("G.O.W: Data/Regions/Felucca.xml does not exist");
                    Utility.PushColor(ConsoleColor.DarkGray);
                    ConsoleLog.Write("................................");
                    Utility.PushColor(ConsoleColor.Red);
                    ConsoleLog.WriteLine("[Failed]");
                ...
                Utility.PushColor(ConsoleColor.White);
                ConsoleLog.Write("G.O.W: Loading Regions Folder");
                Utility.PushColor(ConsoleColor.DarkGray);
                ConsoleLog.Write(".........................................");
                Utility.PopColor();

                XmlDocument xmlDocument = new XmlDocument();
                xmlDocument.Load(Path.Combine(Core.BaseDirectory, "Data/Regions/Trammel.xml"));
                XmlElement xmlElement = xmlDocument["TrammelRegions"];

                XmlDocument xmlDocument2 = new XmlDocument();
                xmlDocument2.Load(Path.Combine(Core.BaseDirectory, "Data/Regions/Felucca.xml"));
                XmlElement xmlElement2 = xmlDocument2["FeluccaRegions"];
                ...
                if (xmlElement == null)
                {
                    Utility.PushColor(ConsoleColor.Red);
                    ConsoleLog.WriteLine("[Failed]");
                    ConsoleLog.WriteLine("G.O.W: Could not find root element 'TrammelRegions' in Trammel.xml");
                    Utility.PopColor();
                    return;
                }
                ...
                {
                    Utility.PushColor(ConsoleColor.Red);
                    Console.WriteLine("Could not find root element 'FeluccaRegions' in Felucca.xml or Towns folder");
                    Utility.PopColor();
                }
                foreach (XmlElement facet in xmlElement.SelectNodes("Facet"))
                {
                    ...
                    if (Region.ReadMap(facet, "name", ref map))
                    {
                        if (map == Map.Internal)
                        {
                            ...
                            ConsoleLog.WriteLine("G.O.W: Invalid internal map in a facet element of Regions");
                            Utility.PopColor();
                        }
                        else
                        {
                            Region.LoadRegions(facet, map, null);
                        }
                    }
                }
                foreach (XmlElement facet in xmlElement2.SelectNodes("Facet"))
                {
                    Map map = null;
            ...
            else
            {
                ...
                {
                    Utility.PushColor(ConsoleColor.White);
                    ConsoleLog.Write("G.O.W: Data/Regions.xml does not exist");
                    Utility.PushColor(ConsoleColor.DarkGray);
                    ConsoleLog.Write("................................");
                    Utility.PushColor(ConsoleColor.Red);
                    ConsoleLog.WriteLine("[Failed]");
                ...
                Utility.PushColor(ConsoleColor.White);
                ConsoleLog.Write("G.O.W: Loading Regions");
                Utility.PushColor(ConsoleColor.DarkGray);
                ConsoleLog.Write("................................................");
                Utility.PopColor();
                ...
                if (xmlElement == null)
                {
                    Utility.PushColor(ConsoleColor.Red);
                    ConsoleLog.WriteLine("[Failed]");
                    ConsoleLog.WriteLine("G.O.W: Could not find root element 'ServerRegions' in Regions.xml");
                    Utility.PopColor();
                    return;
                }
                foreach (XmlElement xml in xmlElement.SelectNodes("Facet"))
                {
                    Map map = null;
                    if (Region.ReadMap(xml, "name", ref map))
                    {
                        if (map == Map.Internal)
                        {
                            Utility.PushColor(ConsoleColor.Yellow);
                            ...
                            ConsoleLog.WriteLine("G.O.W: Invalid internal map in a facet element of Regions.xml");
                            Utility.PopColor();
                        }
                        else
                        {
                            Region.LoadRegions(xml, map, null);
                        }
                    }
                }
                if (flag)
        ...
        {
            			bool flag = true;
			foreach (XmlElement xmlElement in xml.SelectNodes("region"))
			{
				Type defaultRegionType = Region.DefaultRegionType;
				Region.ReadType(xmlElement, "type", ref defaultRegionType, false);

				if (!typeof(Region).IsAssignableFrom(defaultRegionType))
				{
					Utility.PushColor(ConsoleColor.Yellow);
					if (flag)
					{
					...
					ConsoleLog.WriteLine("G.O.W: Invalid region type '{0}' in Regions.xml", new object[]
					{
						defaultRegionType.FullName
					});
					Utility.PopColor();
				}
				else
				{
					...
					try
					{
						region = (Region)Activator.CreateInstance(defaultRegionType, new object[]
						{
							xmlElement,
							map,
							parent
					...
					catch (Exception ex)
					{
						...
						ConsoleLog.WriteLine("G.O.W: Error occured during the creation of region type '{0}': {1}", new object[]
						{
							defaultRegionType.FullName,
							ex
						});
					...
					region.Register();
					Region.LoadRegions(xmlElement, map, region);
				}
			}
        }
        public Region(XmlElement xml, Map map, Region parent)
        {
            ...
            {
                Utility.PushColor(ConsoleColor.Yellow);
                ConsoleLog.WriteLine("G.O.W: Empty area for region '{0}' in Regions.xml", new object[]
				{
					this
				});
            }
        ...
        protected static string GetAttribute(XmlElement xml, string attribute, bool mandatory)
        {
            ...
            {
                Utility.PushColor(ConsoleColor.Yellow);
                ConsoleLog.WriteLine("G.O.W: Missing attribute '{0}' in element '{1}' in Regions.xml", new object[]
				{
					attribute,
					xml.Name
				});
        ...
        public static bool ReadInt32(XmlElement xml, string attribute, ref int value, bool mandatory)
        {
            ...
            {
                Utility.PushColor(ConsoleColor.Yellow);
                ConsoleLog.WriteLine("G.O.W: Could not parse integer attribute '{0}' in element '{1}' in Regions.xml", new object[]
				{
					attribute,
					xml.Name
				});
        ...
        public static bool ReadBoolean(XmlElement xml, string attribute, ref bool value, bool mandatory)
        {
            ...
            {
                Utility.PushColor(ConsoleColor.Yellow);
                ConsoleLog.WriteLine("G.O.W: Could not parse boolean attribute '{0}' in element '{1}' in Regions.xml", new object[]
				{
					attribute,
					xml.Name
				});
        ...
        public static bool ReadDateTime(XmlElement xml, string attribute, ref DateTime value, bool mandatory)
        {
            ...
            {
                Utility.PushColor(ConsoleColor.Yellow);
                ConsoleLog.WriteLine("G.O.W: Could not parse DateTime attribute '{0}' in element '{1}' in Regions.xml", new object[]
				{
					attribute,
					xml.Name
				});
        ...
        public static bool ReadTimeSpan(XmlElement xml, string attribute, ref TimeSpan value, bool mandatory)
        {
            ...
            {
                Utility.PushColor(ConsoleColor.Yellow);
                ConsoleLog.WriteLine("G.O.W: Could not parse TimeSpan attribute '{0}' in element '{1}' in Regions.xml", new object[]
				{
					attribute,
					xml.Name
				});
        ...
        {
            string attribute2 = Region.GetAttribute(xml, attribute, mandatory);
            if (attribute2 == null)
            {
                return false;
            }
            ...
            T t;
            if (typeFromHandle.IsEnum && Enum.TryParse<T>(attribute2, true, out t))
            {
                value = t;
                return true;
            }
            Utility.PushColor(ConsoleColor.Yellow);
            ConsoleLog.WriteLine("G.O.W: Could not parse {0} enum attribute '{1}' in element '{2}' in Regions.xml", new object[]
			{
				typeFromHandle,
				attribute,
				xml.Name
        ...
        public static bool ReadMap(XmlElement xml, string attribute, ref Map value, bool mandatory)
        {
            ...
            try
            {
                value = Map.Parse(attribute2);
            }
            catch
            {
                Utility.PushColor(ConsoleColor.Yellow);
                ConsoleLog.WriteLine("G.O.W: Could not parse Map attribute '{0}' in element '{1}' in Regions.xml", new object[]
				{
					attribute,
					xml.Name
				});
        ...
        public static bool ReadType(XmlElement xml, string attribute, ref Type value, bool mandatory)
        {
            ...
            try
            {
                type = ScriptCompiler.FindTypeByName(attribute2, false);
            }
            catch
            {
                if (flag)
                ...
                Utility.PushColor(ConsoleColor.Yellow);
                ConsoleLog.WriteLine("Could not parse Type attribute '{0}' in element '{1}' in Regions.xml", new object[]
				{
					attribute,
					xml.Name
				});
            ...
            if (type == null)
            {
                ...
                Utility.PushColor(ConsoleColor.Yellow);
                ConsoleLog.WriteLine("G.O.W: Could not find Type '{0}' in Regions.xml", new object[]
				{
					attribute2
				});
                Utility.PopColor();
