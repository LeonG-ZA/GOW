	...
	public static class World
	{
		...
		public static void NotifyDiskWriteComplete()
		{
			if (m_DiskWriteHandle.Set())
			{
                ...
                Console.WriteLine("G.O.W: Closing Save Files. ");
                Utility.PopColor();
			}
		}
			...
			for (int i = 0; i < count; ++i)
			{
				...
				if (t == null)
				{
					...
					if (!Core.Service)
					{
                        ...
                        Utility.PushColor(ConsoleColor.White);
                        ConsoleLog.WriteLine("G.O.W: Error Type '{0}' was not found. Delete all of those types? (y/n)", new object[]
						{
							typeName
						});
						...
						if (Console.ReadKey(true).Key == ConsoleKey.Y)
						{
                            types.Add(null);
                            Utility.PushColor(ConsoleColor.White);
                            ConsoleLog.Write("World: Loading...");
                            Utility.PopColor();
							continue;
						}
                        Utility.PushColor(ConsoleColor.Red);
                        ConsoleLog.WriteLine("G.O.W: Types will not be deleted. An exception will be thrown.");
                        Utility.PopColor();
					}
					else
					{
                        Utility.PushColor(ConsoleColor.Red);
                        ConsoleLog.WriteLine("G.O.W: Error Type '{0}' was not found.", new object[]
						{
							typeName
						});
                        Utility.PopColor();
		...
		public static void Load()
		{
            ...
            Utility.PushColor(ConsoleColor.White);
            ConsoleLog.Write("G.O.W: Loading World");
			...
			if (failedItems || failedMobiles || failedGuilds || failedData)
			{
                Utility.PushColor(ConsoleColor.Yellow);
                ConsoleLog.WriteLine("[Warning]");
                ConsoleLog.WriteLine("G.O.W: An error was encountered while loading a saved object");
                Utility.PopColor();

                ConsoleLog.WriteLine(" - Type: {0}", new object[]
				{
					failedType
				});
                if (failedSerial != Serial.Zero)
                {
                    ConsoleLog.WriteLine(" - Serial: {0}", new object[]
					{
						failedSerial
					});
                }
                ...
                */
				if (!Core.Service)
				{
                    ConsoleLog.WriteLine("Delete the object? (y/n)");
					...
					if (Console.ReadKey(true).Key == ConsoleKey.Y)
					{
						...
						{
                            ConsoleLog.WriteLine("Delete all objects of that type? (y/n)");
				...
				{
                    Utility.PushColor(ConsoleColor.Red);
                    ConsoleLog.WriteLine("G.O.W: An exception will be thrown and the server will terminate.");
                    Utility.PopColor();
				}
            ...
            ConsoleLog.WriteLog(text.Substring(0, num15));
		...
		{
			string message = String.Format("Warning: Attempted to {1} {2} during world save." + "{0}This action could cause inconsistent state." +
