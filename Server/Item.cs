...
namespace Server
{
	...
	public class Item : IEntity, IHued, IComparable<Item>, ISerializable, ISpawnable
	{
        ...
        {
            writer.Write(11); // old version 11
            #region NoTrade
		...
		public virtual void HandleInvalidTransfer(Mobile from)
		{
            ...
            if (NoTrade)
            {
                from.SendMessage(0x22, "That item cannot be traded or dropped. It must remain in your backpack or bank box.");
            }
		}
		...
		{
            //Restore weight of arrows and bolts if they are reduced testing Fix
		...
		public virtual bool DropToMobile(Mobile from, Mobile target, Point3D p)
		{
            ...
            else if (NoTrade && from.Player && !from.IsStaff())
            {
                HandleInvalidTransfer(from);
                return from.AddToBackpack(this);
            }
			else
			{
				return true;
        ...
        public virtual bool OnDroppedInto(Mobile from, Container target, Point3D p, byte gridloc)
		{
            ...
            else if (NoTrade && from.Player)
            {
                if (target == from.Backpack || target == from.BankBox )
                {
        ...
        public virtual bool OnDroppedOnto(Mobile from, Item target)
		{
			...
			else if (NoTrade && Nontransferable && from.Player && target != from.Backpack && !from.IsStaff())
			{
				HandleInvalidTransfer(from);
				return false;
			}
        ...
        public virtual bool CanInsureItem { get { return true; } }
        ...
        public bool EquippedExists(Item item)
        {
